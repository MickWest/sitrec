...
export class CFileManager extends CManager {
  constructor() {
    ...
    if (!isConsole) {
      this.guiFolder = gui
        .addFolder(`FileManager (User:${Globals.userID})`)
        .perm()
        .close();
      ...
      if (Globals.userID > 0) {
        ...
        this.guiFolder
          .add(this, 'openDirectory')
          .name('Open Local Sitch folder')
          .perm();
      }
      ...
      fetch(`${SITREC_SERVER}getsitches.php?get=myfiles`, { mode: 'cors' })
        ...
        .then((data) => {
          ...
          const localfiles = JSON.parse(data); // will give an array of local files
        });
    }
  }
  ...
  loginAttempt(
    ...
    asyncCheckLogin().then(() => {
      ...
      window.open(
        'https://www.metabunk.org/login?_xfRedirect=https://www.metabunk.org/sitrec/sitrecServer/successfullyLoggedIn.html  ',
        '_blank'
      );
      ...
      for await (const entry of this.directoryHandle.values()) {
        ...
        if (entry.name.startsWith('Sit') && entry.name.endsWith('.js')) {
          if (this.localSitchEntry === undefined) {
            this.guiFolder
              .add(this, 'rehostSitch')
              .name('Rehost Local Sitch')
              .perm();
          }
    ...
    this.iterate((key, parsed) => {
      ...
      if (f.staticURL === null) {
        Rehoster.rehostFile(f.filename, f.original).then((staticURL) => {
          ...
          console.log(
            `replacing ${rehostedFilenameQuoted} with ${rehostedURLQuoted}`
          );
          sitchString = sitchString.replaceAll(
            rehostedFilenameQuoted,
            rehostedURLQuoted
          );
          ...
          sitchString = sitchString.replaceAll(
            rehostedFilenameSingleQuoted,
            rehostedURLSingleQuoted
          );
        });
      }
    });
  ...
  loadAsset(filename, id) {
    ...
    const isUrl = isHttpOrHttps(filename);
    if (!isUrl) {
    ...
    Globals.parsing++;
    console.log(
      `>>> loadAsset() Loading Started: ${filename} GlobPars=${Globals.parsing} id=${id}`
    );

    let bufferPromise = null;
    if (!isUrl && isConsole) {
      // read the asset from the local filesystem if this is not running inside a browser
      bufferPromise = import('node:fs').then((fs) => {
        return fs.promises.readFile(filename);
      });
    } else {
      bufferPromise = fetch(`${filename}?v=1${versionString}`).then(
        (response) => {
          if (!response.ok) {
    ...
    return bufferPromise
      ...
      .then((parsedAsset) => {
        ...
        if (Array.isArray(parsedAsset)) {
          assert(
            parsedAsset.length === 1,
            'Zipped IDed asset contains multiple files'
          );
        ...
        Globals.parsing--;
        console.log(
          `<<< loadAsset() parsing Finished: ${filename} GlobPars=${Globals.parsing} id=${id}`
        );
        return parsedAsset; // Return the asset for further chaining if necessary
      })
      .catch((error) => {
  ...
  parseAsset(filename, id, buffer) {
          ...
          const filePromises = Object.keys(zipContents.files).map(
            (zipFilename) => {
              ...
              if (!zipEntry.dir) {
                ...
                return zipEntry.async('arraybuffer').then((unzippedBuffer) => {
    ...
    const fileExt = this.deriveExtension(filename);
      ...
      if (NodeMan.exists('video')) {
        ...
        if (videoNode.Video !== undefined) {
          ...
          if (videoDroppedData !== undefined) {
            ...
            // // start rehosting
            rehostPromises.push(
              Rehoster.rehostFile(rehostFilename, videoDroppedData).then(
                (staticURL) => {
    ...
    Object.keys(this.list).forEach((key) => {
      ...
      if (f.dynamicLink && !f.staticURL) {
        ...
        if (key === 'starLink') {
          ...
          rehostFilename = `${key}-${todayDateStr}.${this.deriveExtension(
            f.filename
          )}`;
          console.log(
            `this.rehostedStarlink set as REHOSTING starLink as ${rehostFilename}`
          );
        } else {
          ...
          if (f.isTLE) {
            this.rehostedStarlink = true;
            console.log(
              `this.rehostedStarlink set as REHOSTING TLE ${rehostFilename}`
            );
          }
        }

        console.log(`Dynamic Rehost: ${rehostFilename}`);
        const rehostPromise = Rehoster.rehostFile(
          rehostFilename,
          f.original
        ).then((staticURL) => {
...
export function createCustomModalWithCopy(url) {
  ...
  addModalButton('Copy URL', () => {
    writeToClipboard(url);
    closeModal();
  });

  addModalButton('Copy & Open', () => {
    writeToClipboard(url);
    closeModal();
    par.paused = true;
