...
class CNodeSwitch extends CNode {
  constructor(v, _gui) {
    ...
    assert(
      this.choice === null || this.inputs[this.choice] !== undefined,
      `CNodeSwitch: choice not found in inputs, choice=${this.choice}`
    );
    ...
    if (this.gui !== undefined) {
      ...
      this.controller = this.gui
        .add(this, 'choice', this.guiOptions)
        .name((isLocal ? '*' : '') + v.desc)
        .onChange((newValue) => {
          // using ()=> preserves this
      ...
      console.warn(
        'No gui for CNodeSwitch - this is probably not what you want'
      );
    }
    this.recalculate();
