...
export class CNodeView3D extends CNodeViewCanvas {
  constructor(v) {
    assert(
      v.camera !== undefined,
      `Missing Camera creating CNodeView 3D, id=${v.id}`
    );
    ...
    console.log(
      ` devicePixelRatio = ${window.devicePixelRatio} canvas.width = ${this.canvas.width} canvas.height = ${this.canvas.height}`
    );
    console.log(
      `Window inner width = ${window.innerWidth} height = ${window.innerHeight}`
    );
  ...
  setupRenderPipeline(v) {
    ...
    this.renderer.setPixelRatio(
      this.in.canvasWidth ? 1 : window.devicePixelRatio
    );
  ...
  renderTargetAndEffects() {
    if (this.visible) {
      ...
      if (this.in.canvasWidth !== undefined) {
        Globals.renderTargetAntiAliased.setSize(
          this.in.canvasWidth.v0,
          this.in.canvasHeight.v0
        );
        if (this.effectsEnabled) {
          // often don't have effects on the main view
          // so we don't need to create/resize these render targets
          Globals.renderTargetA.setSize(
            this.in.canvasWidth.v0,
            this.in.canvasHeight.v0
          );
          Globals.renderTargetB.setSize(
            this.in.canvasWidth.v0,
            this.in.canvasHeight.v0
          );
        }
      ...
      if (this.canDisplayNightSky && GlobalNightSkyScene !== undefined) {
        ...
        const tempPos = this.camera.position.clone();
      ...
      if (this.effectsEnabled) {
        ...
        for (const effectName in this.effectPasses) {
          ...
          const useRenderTarget =
            currentRenderTarget === Globals.renderTargetA
              ? Globals.renderTargetB
              : Globals.renderTargetA;
  ...
  addEffects(effects) {
    if (effects) {
      ...
      for (const effectKey in this.effects) {
        const def = this.effects[effectKey];
        ...
        console.log(
          `Adding effect kind${effectKind} id=${effectID}  to ${this.id}`
        );

        // create the node, which will wrap a .pass member which is the ShaderPass
        this.effectPasses.push(
          new CNodeEffect({
            id: effectID,
  ...
  addFocusTracks(focusTracks) {
    ...
    this.lockTrackName = 'default';
    gui
      .add(this, 'focusTrackName', focusTracks)
      .onChange((focusTrackName) => {
        //
      })
      .name('Focus Track')
      .listen();
    gui
      .add(this, 'lockTrackName', focusTracks)
      .onChange((lockTrackName) => {
        //
  ...
  renderCanvas(frame) {
    ...
    const rgb = new Color(this.background);
  ...
  onMouseDown(event, mouseX, mouseY) {
    ...
    const mouseYUp = this.heightPx - (mouseY - this.topPx);
    const mouseRay = makeMouseRay(this, mouseX, mouseYUp);
    ...
    if (event.button === 1 && this.camera) {
      ...
      if (NodeMan.exists('ufoSplineEditor')) {
        ...
        if (ufoSpline.enable) {
          ...
          const closest = ufoSpline.closestPointToRay(
            this.raycaster.ray
          ).position;
    ...
    if (
      this.id === 'mainView' &&
      this.camera &&
      mouseInViewOnly(this, mouseX, mouseY)
    ) {
      this.raycaster.setFromCamera(mouseRay, this.camera);
  ...
  onMouseMove(event, mouseX, mouseY) {
    ...
    if (this.mouseDown) {
      if (this.dragMode > 0) {
        ...
        //  debugText = ""
        const closestPoint = V3();
        let distance = 10000000000;
        let found = false;
        const spherePointWorldPosition = V3();
        if (this.dragMode === 1)
          glareSphere.getWorldPosition(spherePointWorldPosition);
        else targetSphere.getWorldPosition(spherePointWorldPosition);

        for (let i = 0; i < intersects.length; i++) {
          if (intersects[i].object.name === 'dragMesh') {
            const sphereDistance = spherePointWorldPosition.distanceTo(
              intersects[i].point
            );
        ...
        if (found) {
          const closestPointLocal = LocalFrame.worldToLocal(
            closestPoint.clone()
          );
          if (this.dragMode === 1) {
            // dragging green
            ...
            par.globalRoll = roll;
            par.podRollPhysical =
              par.globalRoll - NodeMan.get('bank').v(par.frame);
            ChangedPR();
      ...
      if (found) {
        ...
        if (1 || this.useGlobe) {
          const dragSphere = new Sphere(
            new Vector3(0, -wgs84.RADIUS, 0),
            wgs84.RADIUS /* + f2m(this.defaultTargetHeight) */
          );
          ...
          const selectPlane = new Plane(
            new Vector3(0, -1, 0),
            f2m(this.defaultTargetHeight)
          );
      ...
      if (this.focusTrackName !== 'default') {
        ...
        const closestFrame = focusTrackNode.closestFrameToRay(
          this.raycaster.ray
        );
