...
export class CNodeDisplaySkyOverlay extends CNodeViewUI {
  constructor(v) {
    ...
    guiShowHide
      .add(this, 'showStarNames')
      .onChange(() => {
        par.renderOne = true;
      })
      .name(`${this.overlayView.id} Star names`);
  }
  ...
  renderCanvas(frame) {
    ...
    if (this.showStarNames) {
      for (const HR in this.nightSky.commonNames) {
        ...
        if (
          ...
          let x = ((pos.x + 1) * this.widthPx) / 2;
          let y = ((-pos.y + 1) * this.heightPx) / 2;
          x += 5;
          y -= 5;
      ...
      for (const [name, planet] of Object.entries(
        this.nightSky.planetSprites
      )) {
    ...
    if (this.showSatelliteNames && this.nightSky.TLEData) {
      ...
      for (const [index, sat] of Object.entries(
        this.nightSky.TLEData.satrecs
      )) {
        ...
        if (positionAndVelocity?.position) {
          ...
          const ecefK = satellite.eciToEcf(positionEci, gmst);
...
function fixTLELine(line, ends) {
  ...
  if (expectedFields === 9) {
    ...
    if (fields.length > 8) {
      ...
      assert(
        fields.length === 9,
        `TLE line 2 has too many fields: ${line} ${fields.length} ${expectedFields}`
      );
  ...
  assert(
    ...
    `TLE line does not have the right number of fields: ${line} ${fields.length} ${expectedFields}`
  );
  ...
  for (let i = 0; i < expectedFields; i++) {
    ...
    const expectedLength = ends[i];
    let field = fields[i];
    // this is how long it would be if we just added this string
    const actualLength = newLine.length + field.length;
    ...
    assert(
      ...
      `TLE field ${i} is not the right length: ${newLine}`
    );
  }
  //   console.log(line);
  //   console.log(newLine);
...
export class CNodeDisplayNightSky extends CNode3DGroup {
  constructor(v) {
    ...
    this.flareAngle = 5;
    guiTweaks
      .add(this, 'flareAngle', 0, 20, 0.1)
      .listen()
      .name('SL Flare Angle');

    this.penumbraDepth = 5000;
    guiTweaks
      .add(this, 'penumbraDepth', 0, 100000, 1)
      .listen()
      .name("Earth's Penumbra Depth");
  ...
  update(frame) {
    ...
    const observer = new Astronomy.Observer(Sit.lat, Sit.lon, 0);
  ...
  updateSatelliteScales(camera) {
    ...
    if (camera.satTimeStep === undefined) {
      ...
      camera.satStartTime++;
      if (camera.satStartTime >= camera.satTimeStep) camera.satStartTime = 0;
    }
    ...
    this.starMaterial.uniforms.starScale.value =
      Sit.starScale / window.devicePixelRatio;
    ...
    if (this.showSatellites && this.TLEData) {
      ...
      this.satelliteMaterial.uniforms.satScale.value =
        Sit.satScale / window.devicePixelRatio;
      ...
      for (let i = camera.satStartTime; i < this.TLEData.satrecs.length; i++) {
        ...
        if (
          (i - camera.satStartTime) % camera.satTimeStep !== 0 &&
          !sat.hasArrow
        )
          continue;
        ...
        assert(
          sat.eus !== undefined,
          `sat.eus is undefined, i= ${i}, this.TLEData.satrecs.length = ${this.TLEData.satrecs.length} `
        );

        const satPosition = sat.eus;

        let scale = 0.1; // base value for scale
        const darknessMultiplier = 0.3; // if in dark, multiply by this
        let fade = 1;
        ...
        if (fade > 0) {
          ...
          const belowHorizon = intersectSphere2(
            raycaster.ray,
            this.globe,
            hitPoint,
            hitPoint2
          ...
          if (!belowHorizon) {
            const globeToSat = satPosition
              .clone()
              .sub(this.globe.center)
              .normalize();
            ...
            const altitudeKM =
              (satPosition.clone().sub(this.globe.center).length() -
                wgs84.RADIUS) /
              1000;
            ...
            if (glintAngle < spread) {
              ...
              const arrowHelper = DebugArrowAB(
                sat.name,
                this.camera.position,
                satPosition,
                belowHorizon ? '#303030' : '#FF0000',
                true,
                this.sunArrowGroup,
                10,
                LAYER.MASK_HELPERS
              );
              const arrowHelper2 = DebugArrowAB(
                `${sat.name}sun`,
                satPosition,
                satPosition.clone().add(toSun.clone().multiplyScalar(10000000)),
                '#c08000',
                true,
                this.sunArrowGroup,
                10,
        ...
        if (scale < Sit.satCutOff) scale = 0;
  ...
  loadStarData() {
    ...
    while (offset < -starn * nbent - 28) {
      ...
      offset += 8;
      const is = utf8decoder.decode(
        new Uint8Array([view.getUint8(offset), view.getUint8(offset + 1)])
      );
      offset += 2;
      ...
      if (mag > this.BSC_MaxMag) this.BSC_MaxMag = mag;
    }
  }
    ...
    for (const line of lines) {
      if (line[0] === '#') {
        ...
        if (designation.startsWith('HR')) {
          const hr = Number.parseInt(designation.substring(3));
  ...
  addCelestialSphereLines(scene, gap = 15, color = 0x808080) {
    ...
    for (let dec = -90; dec <= 90; dec += gap) {
      ...
      const line = new Line(
        geometry,
        dec === 90 - gap ? materialWhite : material
      );
      scene.add(line);
    }
  }
  ...
  addStars(scene) {
    ...
    // Setup the sprite material
    const spriteMap = new TextureLoader().load(
      'data/images/nightsky/MickStar.png'
    ); // Load a star texture
    ...
    const magnitudes = new Float32Array(numStars); // magnitude for each star

    for (let i = 0; i < numStars; i++) {
      // Convert RA, Dec to 3D position
      const equatorial = raDec2Celestial(
        this.BSC_RA[i],
        this.BSC_DEC[i],
        sphereRadius
      );
      ...
      const mag = this.BSC_MAG[i]; // Magnitude
      const scale = ((this.BSC_MaxMag + 0.5 - mag) * 0.1) ** 3;
      //   scale *= Sit.starScale ?? 1;
...
void main() {
  ...
  //  if (alpha < 0.0) discard; // Gives a circular shape
    ...
    this.starMaterial = new ShaderMaterial({
      ...
      uniforms: {
        ...
        maxSize: { value: 20.0 },
        starTexture: {
          value: new TextureLoader().load('data/images/nightsky/MickStar.png'),
        },
        cameraFOV: { value: 30 },
  ...
  addPlanets(scene) {
    ...
    const starMap = new TextureLoader().load(
      'data/images/nightsky/MickStar.png'
    ); // Load a star texture

    const sunMap = new TextureLoader().load('data/images/nightsky/MickSun.png'); // Load a star texture

    const moonMap = new TextureLoader().load(
      'data/images/nightsky/MickMoon.png'
    ); // Load a star texture
...
2 25544  51.6441 179.2338 0008176  49.9505 310.1752 15.48903444320729
  ...
  addSatellites(scene, textGroup) {
    ...
    const positions = new Float32Array(this.TLEData.satrecs.length * 3); // x, y, z for each satellite
    const colors = new Float32Array(this.TLEData.satrecs.length * 3); // r, g, b for each satellite
    const magnitudes = new Float32Array(this.TLEData.satrecs.length); // magnitude for each satellite
    ...
    // Attach data to geometry
    this.satelliteGeometry.setAttribute(
      'position',
    ...
    this.satelliteGeometry.setAttribute(
      'magnitude',
      new BufferAttribute(magnitudes, 1)
    );

    // Create point cloud for satellites
    this.satellites = new Points(
  ...
  calcSatUES(sat, date) {
    ...
    if (positionAndVelocity?.position) {
      ...
      const sphericalECEF = llaToSphericalECEF(
        llaPos.lat,
        llaPos.lon,
        llaPos.alt
      );
  ...
  updateAllSatellites(date) {
    ...
    for (let i = 0; i < numSats; i++) {
      ...
      if (sat.eusA !== null && sat.eusB !== null) {
        ...
        const velocity = sat.eusB
          .clone()
          .sub(sat.eusA)
          .multiplyScalar(1000 / (sat.timeB - sat.timeA))
          .length();
        ...
        if (velocity < 5000 || velocity > 11000) {
          ...
          const t = (timeMS - sat.timeA) / (sat.timeB - sat.timeA);
          ...
          if (this.showSatelliteTracks) {
            const A = sat.eusA.clone();
            const dir = sat.eusB.clone().sub(sat.eusA).normalize();
            DebugArrow(
              `${sat.name}_t`,
              dir,
              A,
              ...
              '#FFFF00',
              true,
              this.satelliteTrackGroup,
              20,
              LAYER.MASK_LOOKRENDER
            );
          }
  ...
  updatePlanetSprite(planet, sprite, date, observer, sphereRadius) {
    ...
    const celestialInfo = Astronomy.Equator(
      planet,
      date,
      observer,
      false,
    ...
    if (planet === 'Sun') {
      ...
      const { az: az1, el: el1 } = raDecToAzElRADIANS(
        ra,
        dec,
        radians(LLA.x),
        radians(LLA.y),
        getLST(date, radians(LLA.y))
      );
      const { az, el } = raDecToAltAz(
        ra,
        dec,
        radians(LLA.x),
        radians(LLA.y),
      ...
      let bestX = X0;
      let bestGlintAngle = 100000; // large value so the first one primes it
      let bestAngle = 0;
      ...
      do {
        ...
        for (let angle = start; angle <= end; angle += step) {
          ...
          const X = X0.clone()
            .sub(O)
            .applyAxisAngle(axis, radians(angle))
            .add(O);
...
export function addNightSky(def) {
  ...
  const nightSky = new CNodeDisplayNightSky({ id: 'NightSkyNode', ...def });
