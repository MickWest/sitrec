...
class CNodeSparseTrack extends CNode {
  constructor(v) {
    ...
    assert(
      v.times.length === v.positions.length,
      `CNodeSparseTrack array length mismatch, times=${v.times.length}, positions=${v.positions.length}`
    );
...
class CNodeSplineTrack extends CNodeEmptyArray {
  ...
  getLength(numSteps) {
    ...
    for (let i = 1; i < numSteps; i++) {
      const t = i / (numSteps - 1); // go from 0 to 1, so we need steps-1 for the last one
