...
export class CNodeControllerSaucerTilt extends CNodeController {
  ...
  apply(f, objectNode) {
    ...
    if (object !== undefined) {
      if (f >= 0) {
        const next = this.in.track.p(f + 1);
        ...
        const pos = this.in.track.p(f);
        let next = this.in.track.p(f + 1);
        ...
        switch (this.tiltType.toLowerCase()) {
          case 'banking': {
            ...
            // first get the
            const velocityA = trackDirection(
              this.in.track,
              f - (sampleDuration * Sit.fps) / 2
            );
            const velocityB = trackDirection(
              this.in.track,
              f + (sampleDuration * Sit.fps) / 2
            );
            ...
            if (right) angularVelocity = -angularVelocity;
          ...
          case 'axialpushzerog': {
            ...
            const accelerationDir = trackAcceleration(this.in.track, f);

            if (this.tiltType === 'axialPull') accelerationDir.negate();

            if (
              this.tiltType === 'axialPull' ||
              this.tiltType === 'axialPush'
            ) {
          ...
          case 'bottompointingair': {
            const from = this.in.airTrack.p(f);
            const to = this.in.airTrack.p(f + 1);
            const fwdAir = to.sub(from);
          ...
          case 'glareangle': {
            ...
            const glare = radians(getGlareAngleFromFrame(f) + 90);
            const mg = new Matrix4();
            // get LOS from the camera to the target
            const to = this.in.airTrack.p(f);

            if (NodeMan.exists('jetTrack')) {
              const from = NodeMan.get('jetTrack').p(f);
              const fwdLOS = to.clone().sub(from).normalize();
              ...
              console.warn(
                'jetTrack not found for glare angle in CNodeDisplayTargetModel.js'
              );
            }
            break;
          }
  ...
  pointBottomAt(object, pos, next) {
    ...
    // debug only
    const direction = next.clone().sub(pos);
    direction.normalize();
