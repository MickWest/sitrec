...
export class CNodeVideoWebCodecView extends CNodeVideoView {
  constructor(v) {
    ...
    if (!v.overlayView) {
      ...
      this.overlay = new CNodeViewUI({
        id: `${this.id}_videoOverlay`,
        overlayView: this,
      });
    ...
    if (v.file === undefined) {
      ...
      this.Video = new CVideoWebCodecData(
        v,
  ...
  addDownloadButton() {
    ...
    function getAbsolutePath(url, root) {
      ...
      if (url.startsWith('../')) {
        ...
        const lastSlash = root.lastIndexOf('/', root.length - 2);
    ...
    this.button = FileManager.guiFolder
      .add(obj, 'openURL')
      .name('Download Video');
  }
  ...
  uploadFile(file) {
    ...
    this.addLoadingMessage();
    this.Video = new CVideoWebCodecData(
      { id: `${this.id}_data`, dropFile: file },
      this.loadedCallback.bind(this),
      this.errorCallback.bind(this)
    );
    par.frame = 0;
