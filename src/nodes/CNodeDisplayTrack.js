  ...
  recalculate() {
    ...
    for (let f = 0; f < this.frames; f++) {
      ...
      assert(
        ...
        `CNodeDisplayTrack: trackPoint is undefined, id=${this.id} frame=${f}`
      );
      ...
      if (trackPoint?.position !== undefined) {
        ...
        assert(
          !Number.isNaN(A.x) && !Number.isNaN(A.y) && !Number.isNaN(A.z),
          `CNodeDisplayTrack: trackPoint has NaNs in position, id=${this.id} frame=${f}`
        );

        line_points.push(A.x, A.y, A.z);
        let color = trackPoint.color; // the track itself can override the color defaults
        if (color === undefined) {
          ...
          if (trackPoint.bad)
            if (this.in.badColor !== undefined)
              color =
                this.in.badColor.v(f); // display can specify a "bad" color
            else color = { r: 1, g: 0, b: 0 }; // "bad" default color is red
        }

        if (!this.ignoreAB && (f < Sit.aFrame || f > Sit.bFrame)) {
          if (this.in.secondColor !== undefined)
            color = this.in.secondColor.v(f);
          else color = { r: 0.25, g: 0.25, b: 0.25 };
        }
        ...
        if (this.toGround !== undefined && this.toGround > 0) {
          if (f % this.toGround === 0) {
            const groundY = 0 - drop(A.x, A.z, wgs84.RADIUS);
    ...
    */
    ...
    assert(
      ...
      `CNodeDisplayTrack: no points in track ${this.id}`
    );
    ...
    let width = 1;
    if (this.in.width !== undefined) width = this.in.width.v0;

    const matLineTrack = new LineMaterial({
      color: 0xffffff,
      //   color: 0xff0000,
