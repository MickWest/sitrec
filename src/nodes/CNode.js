...
class CNode {
  ...
  constructor(v) {
    ...
    if (v.id !== undefined) {
      ...
      console.log(
        `Node with unique ID: ${this.id} created, class = ${this.constructor.name}`
      );
  ...
  dispose() {
    ...
    for (const output of this.outputs) {
      output.inputs = Object.fromEntries(
        Object.entries(output.inputs).filter(([key, value]) => value !== this)
      );
    }
    this.inputs = {};
    this.outputs = [];
  ...
  update(f) {
    ...
    assert(
      f !== undefined,
      "Something upstream is not passing in f to node's update function"
    );
  }
  ...
  hideInactiveSources() {
    ...
    for (const key in this.inputs) {
      ...
      if (input.countVisibleOutputs() === 0) {
        if (this.visible)
          console.log(`hideInactiveSources: Hiding ${input.id}`);
        input.hide();
      }
    }
  ...
  showActiveSources() {
    ...
    for (const key in this.inputs) {
      ...
      if (input.countVisibleOutputs() > 0) {
        if (!this.visible)
          console.log(`showActiveSources: Showing ${input.id}`);
        input.show();
      }
    }
  ...
  setGUI(v, _gui) {
    ...
    if (_gui) {
      ...
      if (typeof _gui === 'string') {
        ...
        assert(
          _gui.constructor.name === 'GUI',
          'CNodeSwitch: gui must be a lil-gui object'
        );
        this.gui = _gui;
      }
    } else if (gui) {
  ...
  checkInputs(inputList) {
    inputList.forEach((key) =>
      assert(this.inputs[key] !== undefined, `CNode Missing input -> ${key}`)
    );
  }

  // check that we have one and only one of a list of inputs
  checkExclusiveInputs(inputList) {
    let numMatchingInputs = 0;
    inputList.forEach((key) => {
      if (this.inputs[key] !== undefined) numMatchingInputs++;
    });
  ...
  addInput(key, nodeID, optional = false) {
    assert(
      this.in.key === undefined,
      `Adding input ${key} that is already defined`
    );
    let node;
    if (nodeID instanceof CNode) {
      ...
      if (typeof nodeID !== 'number') {
        if (optional) {
          ...
          if (!NodeMan.exists(nodeID)) return;
        }

        node = NodeMan.get(nodeID);
        assert(
          node instanceof CNode,
          `Non-Node with id=${nodeID}  found for input key=${key}`
        );
      } else {
  ...
  input(i, optional = false) {
    ...
    if (this.inputs[i] !== undefined) {
      ...
      assert(
        this.inputs[i] instanceof CNode,
        `Node has none-node input -> ${i}`
      );
      return;
    }
    ...
    if (this.props[i] instanceof CNode) {
      ...
      if (NodeMan.exists(this.props[i]))
        ...
        assert(
          typeof this.props[i] === 'number',
          `Node ${this.id} has input ${i} : ${this.props[i]} which is not a node or number, probably name of noded that's not created`
        );
  ...
  requireInputs(inputList) {
    inputList.forEach((key) => this.input(key));
  }

  optionalInputs(inputList) {
    inputList.forEach((key) => this.input(key, true));
  }
  ...
  getValue(frameFloat) {
    ...
    if (!this.isNumber && this.getValueFrame(0).position === undefined) {
      ...
      assert(
        frameInt >= 0 && frameInt < this.frames,
        'out of range index on non-number/non-position'
      );
    ...
    if (numFrames <= 1) {
      ...
      if (frameFloat < 0) {
        ...
        if (value0.position === undefined) {
          ...
          if (typeof value0 === 'number' && typeof value1 === 'number') {
            ...
            value = value1
              .clone()
              .sub(value0)
              .multiplyScalar(frameFloat - (numFrames - 1))
              .add(value1);
          }
        } else {
          value = { ...value0 }; // make a copy, so we can alter the position
        ...
        if (Number.isInteger(frameFloat)) {
          ...
          assert(
            value0 !== undefined,
            `Node ${this.id} has undefined value0 at frame ${frameInt}`
          );
          assert(
            value1 !== undefined,
            `Node ${this.id} has undefined value1 at frame ${frameInt + 1}`
          );
          ...
          if (value0.position === undefined)
            ...
            value.position = value1.position
              .clone()
              .sub(value0.position)
              .multiplyScalar(frameFloat - frameInt)
              .add(value0.position);
          }
        }
      }
  ...
  p(frameFloat) {
    ...
    assert(
      pos !== undefined,
      `Node ${this.id} has undefined value at frameFloat ${frameFloat}`
    );
    ...
    //     console.log(pos);
    // }
    if (pos.position !== undefined) pos = pos.position;
    // assert pos.clone is a function
    assert(
      pos.clone !== undefined,
      `Node ${this.id} has no position at frame ${frameFloat}`
    );
  ...
  getValueFrame() {
    assert(
      0,
      'Should not call getValueFrame base implementation. Node missing getValueFrame implementation?'
    );
  }
  ...
  closestFrameToRay(ray) {
    let closestPoint = V3(); // target.clone() // use the previosul found point, but should be overridden
  ...
  maxDepthOf(node, depth = 0, currentMax = -1) {
    ...
    if (depth > currentMax && this === node) currentMax = depth;
    // otherwise recurse for each child, with a higher depth
    this.outputs.forEach(
      (child) => (currentMax = child.maxDepthOf(node, depth + 1, currentMax))
    );
  ...
  recalculateCascade(f, noControllers = false, depth = 0, debug = false) {
    ...
    if (debug)
      console.log(
        `${'|---'.repeat(depth)} Recalculating:  ${this.id} frame ${f}`
      );

    const listOfOne = [this];
...
function recalculateNodesBreadthFirst(
  list,
  f,
  noControllers,
  depth,
  ...
  for (const node of list) {
    if (debug) console.log(`${'|---'.repeat(depth)} Recalulating:  ${node.id}`);
    node.recalculate();
    ...
    if (!noControllers && node.applyControllers !== undefined) {
      if (debug && node.id === 'lookCamera')
        console.log(
          `${'|---'.repeat(depth)} applyControllers to  ${node.id} frame ${f}`
        );
