...
function startToNowMS(startMS) {
  const nowMS = Math.round(
    startMS + (par.frame * 1000 * (Sit.simSpeed ?? 1)) / Sit.fps
  );
  return nowMS;
}
...
function nowToStartMS(nowMS) {
  const startMS = Math.round(
    nowMS - (par.frame * 1000 * (Sit.simSpeed ?? 1)) / Sit.fps
  );
  return startMS;
}
...
export class CNodeDateTime extends CNode {
  constructor(v) {
    console.log(
      `CNodeDateTime - par.frame = ${par.frame} Sit.fps = ${Sit.fps} Sit.simSpeed = ${Sit.simSpeed} Sit.startTime = ${Sit.startTime}`
    );
    ...
    for (let i = 0; i < 100000; i++) {
      ...
      assert(
        start === start2,
        `start2now now2start error at i = ${i} start = ${start} now = ${now} start2 = ${start2}`
      );
    }
    ...
    this.dateTimeFolder
      .add(this.dateTime, 'year', 1947, 2030, 1)
      .listen()
      .onChange((v) => this.updateDateTime(v));
    this.dateTimeFolder
      .add(this.dateTime, 'month', 1, 12, 1)
      .listen()
      .onChange((v) => this.updateDateTime(v));
    this.dateTimeFolder
      .add(this.dateTime, 'day', 1, 31, 1)
      .listen()
      .onChange((v) => this.updateDateTime(v));
    this.dateTimeFolder
      .add(this.dateTime, 'hour', 0, 23, 1)
      .listen()
      .onChange((v) => this.updateDateTime(v));
    this.dateTimeFolder
      .add(this.dateTime, 'minute', 0, 59, 1)
      .listen()
      .onChange((v) => this.updateDateTime(v));
    this.dateTimeFolder
      .add(this.dateTime, 'second', 0, 59, 1)
      .listen()
      .onChange((v) => this.updateDateTime(v));
    this.dateTimeFolder
      .add(this.dateTime, 'millisecond', 0, 999, 1)
      .listen()
      .onChange((v) => this.updateDateTime(v));
    ...
    this.dateTimeFolder
      .add(this, 'timeZoneName', timeZoneKeys)
    ...
    this.dateTimeFolder
      .add(this, 'resetNowTimeToCurrent')
      .name('Sync to Current Time');
  ...
  addSyncSwitch() {
    this.syncSwitch = this.dateTimeFolder
      .add(this, 'syncMethod', ['-', 'Start Time', 'Now Time'])
      .name('Sync Time to')
      .onChange((v) => {
        if (v === '-') {
          // do nothing
  ...
  AdjustStartTime(t) {
    let time = this.getStartTimeValue();
    time += t;
  ...
  frameToMS(frame) {
    ...
    const MS = Math.round(
      startMS + (frame * 1000 * (Sit.simSpeed ?? 1)) / Sit.fps
    );
    return MS;
  }
  ...
  update(frame) {
    ...
    let speedscale = 1;
    if (isKeyHeld('shift')) speedscale *= 10;
    if (isKeyHeld('control')) speedscale *= 100;
    if (isKeyHeld('alt')) speedscale *= 1000;
    if (isKeyHeld('meta')) speedscale *= 10000;
