...
export function setupMeasurementUI() {
  ...
  console.warn(
    `%%%%%%% setupMeasurementUI: Globals.showMeasurements = ${Globals.showMeasurements}`
  );
  ...
  measurementUIVars.controller = guiShowHide
    .add(Globals, 'showMeasurements')
    .name('Measurements')
    .listen()
    .onChange((value) => {
...
export class CNodeLabel3D extends CNode3DGroup {
  ...
  updateScale(view) {
    ...
    if (sprite.layers.mask & mask) {
      ...
      const scale = distance * fovScale * this.size;
...
export class CNodeMeasureAB extends CNodeLabel3D {
  ...
  update(f) {
    ...
    if (this.altitude) {
      ...
      const alt = altitudeAboveSphere(this.A);
