    ...
    for (let f = 0; f < this.frames; f++) {
      ...
      if (f === 0) {
        ...
        if (typeof lineHeading !== 'number') {
          ...
          assert(!Number.isNaN(lineHeading), 'lineHeadingNAN');

          if (Number.isNaN(lineHeading) && f < 5) {
            console.warn(
              'lineHeading is NaN in CNodeLOSTraverseStraightLine - probably a bug'
            );
          }
        ...
        const plane = new Plane(rightAxis, planeDistance);
        ...
        const target0 = V3(); // there's only one possible intersection of a line and a plane (unless parallel/coplanar)
...
export class CNodeLOSTraverseStraightLineFixed extends CNodeTrack {
  ...
  recalculate() {
    ...
    let startPosition;
    let oldDistance;
    const perFrameMotion = this.in.speed.v(f) / this.fps;
    const radius = metersFromMiles(this.in.radius.v0);
    for (let f = 0; f < this.frames; f++) {
      const los = this.in.LOS.v(f);
