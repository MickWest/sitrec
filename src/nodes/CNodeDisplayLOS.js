...
const matLineGreyThin = makeMatLine(0x404040, 0.5);
...
export class CNodeDisplayLOS extends CNode3DGroup {
  ...
  update() {
    ...
    if (this.in.traverse)
      for (let f = 0; f < this.in.LOS.frames; f++) {
        // one per second (assume 30 fps), plus arbitary lines to highlight
        if (
          this.isFineDetail(f) ||
          f % this.spacing === 0 ||
          this.highlightLines[f] !== undefined
        ) {
          const traverse = this.in.traverse.v(f);
          if (traverse.position) {
            if (this.isFineDetail(f))
              DebugSphere(
                `LOSInteresect${this.detailFrameName(f)}`,
                traverse.position,
                0.2,
                0xff0000
              );
  ...
  recalculate() {
    ...
    for (let f = 0; f < this.in.LOS.frames; f++) {
      ...
      if (
        ...
        const A = los.position.clone();
        const fwd = los.heading.clone();
        let B = A.clone();
        ...
        let color = this.material; // matLineWhiteThin;
