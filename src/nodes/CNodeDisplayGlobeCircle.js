...
export class CNodeDisplayGlobeCircle extends CNode3DGroup {
  ...
  rebuild() {
    ...
    const circleCenter = globeCenter
      .clone()
      .add(this.normal.multiplyScalar(this.offset));
    const circleRadius = Math.sqrt(
      this.radius * this.radius - this.offset * this.offset
    );
    const segments = 100;
    for (let i = 0; i < segments; i++) {
      ...
      point.add(
        perpendicular.clone().multiplyScalar(Math.cos(theta) * circleRadius)
      );
      point.add(
        otherPerpendicular
          .clone()
          .multiplyScalar(Math.sin(theta) * circleRadius)
      );
