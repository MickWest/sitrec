...
export class CNodeTerrainUI extends CNode {
  constructor(v) {
    ...
    this.latController = this.gui
      .add(this, 'lat', -85, 85, 0.001)
      .onChange((v) => {
    ...
    this.lonController = this.gui
      .add(this, 'lon', -180, 180, 0.001)
      .onChange((v) => {
        this.flagForRecalculation();
      })
      .onFinishChange((v) => {
      ...
      .setLabelColor(terrainGUIColor);
    this.zoomController = this.gui
      .add(this, 'zoom', 0, 15, 1)
      .onChange((v) => {
        this.flagForRecalculation();
      })
      .onFinishChange((v) => {
      ...
      .setLabelColor(terrainGUIColor);
    this.nTilesController = this.gui
      .add(this, 'nTiles', 1, 8, 1)
      .onChange((v) => {
        this.flagForRecalculation();
      })
      .onFinishChange((v) => {
  ...
  zoomToTrack(v) {
    ...
    const { minLat, maxLat, minLon, maxLon, minAlt, maxAlt } =
      trackNode.getLLAExtents();
...
export class CNodeTerrain extends CNode {
  constructor(v) {
    ...
    if (!v.mapTypeMenu) {
      ...
      this.mapTypeMenu = this.terrainGUI
        .add(local, 'mapType', mapTypes)
        .setLabelColor(terrainGUIColor)
        .listen()
        .name('Map Type');
    } else {
    ...
    const circumfrence = 40075000 * cos(radians(this.lat));
    this.tileSize = circumfrence / 2 ** this.zoom; // tileSize is the width and height of the tile in meters
    ...
    const tilex = Math.floor(lon2tile(this.position[1], this.zoom)) + 0.5; // this is probably correct
    const tiley = Math.floor(lat2tile(this.position[0], this.zoom)) + 0.5; // this might be a bit off, as not linear?
    const lon0 = tile2lon(tilex, this.zoom);
    const lat0 = tile2lat(tiley, this.zoom);
    ...
    this.loadMap(
      local.mapType,
      this.deferLoad !== undefined ? this.deferLoad : false
    );
    ...
    this.mapTypeMenu.onChange((v) => {
      ...
      for (const mapID in this.maps)
        this.maps[mapID].group.visible = mapID === v;
      this.loadMap(v);
    });
  ...
  recalculate() {
    ...
    if (this.in.flattening !== undefined) {
      let flattening = this.in.flattening.v0;
      if (flattening >= 1) flattening = 0.999999;
      radius *= 1 / (1 - flattening);
    }
    Sit.originECEF = RLLAToECEFV_Sphere(
      radians(Sit.lat),
      radians(Sit.lon),
      0,
      radius
  ...
  getPointBelow(A) {
    ...
    const elevation = this.maps[local.mapType].map.getElevationInterpolated(
      LLA.x,
      LLA.y
    );
