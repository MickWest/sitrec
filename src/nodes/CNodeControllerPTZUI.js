...
export class CNodeControllerPTZUI extends CNodeController {
  ...
  apply(f, objectNode) {
    ...
    let fwd = V3(0, 0, -1); // North, parallel to the local tangent
    let right = V3(1, 0, 0); // East
    let up = V3(0, 1, 0); // up
    ...
    const northPoleECEF = V3(0, 0, wgs84.RADIUS);
    const northPoleEUS = ECEF2EUS(
      northPoleECEF,
      radians(Sit.lat),
      radians(Sit.lon),
      wgs84.RADIUS
    );
    const toNorth = northPoleEUS.clone().sub(camera.position).normalize();
    // take only the component perpendicular
    const dot = toNorth.dot(up);
    const north = toNorth
      .clone()
      .sub(up.clone().multiplyScalar(dot))
      .normalize();
