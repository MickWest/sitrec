...
function updateSitFrames() {
  if (Sit.framesFromVideo) {
    console.log(
      `updateSitFrames() setting Sit.frames to Sit.videoFrames=${Sit.videoFrames}`
    );
...
export class CVideoWebCodecData extends CVideoData {
  ...
  restartWorker() {
    ...
    for (let i = 0; i < this.numWorkers; i++) {
      this.filterWorkers[i] = new Worker(
        `./src/workers/PixelFilterWorker.js?=${versionString}`
      );
  ...
  startWithDemuxer(demuxer) {
    ...
    this.decoder = new VideoDecoder({
      output: (videoFrame) => {
        ...
        while (
          groupNumber + 1 < this.groups.length &&
          videoFrame.timestamp >= this.groups[groupNumber + 1].timestamp
        )
          groupNumber++;
        ...
        createImageBitmap(videoFrame).then((image) => {
          ...
          assert(
            group.decodeOrder !== undefined,
            `Missing decode order, maybe different group for frame: ${frameNumber} timestamp =${videoFrame.timestamp} group.frame = ${group.frame} group.length = ${group.length}`
          );
          ...
          assert(
            group.pending > 0,
            `Decoding more frames than were listed as pending at frame ${frameNumber} timestamp =${videoFrame.timestamp} group.frame = ${group.frame} group.length = ${group.length}`
          );

          if (group.pending <= 0) {
            let framesDecoded = '';
            for (const i in group.decodeOrder) {
          ...
          if (group.pending === 0) {
            ...
            if (this.groupsPending === 0 && this.nextRequest >= 0) {
              console.log(
                `FULFILLING deferred request as no groups pending , frame = ${this.nextRequest}`
              );
        ...
        if (this.YUVFilters) {
          ...
          videoFrame
            .copyTo(buffer)
            .then((this.frameCache[frameNumber] = buffer));
        }
      },
    ...
    demuxer.getConfig().then((config) => {
      ...
      demuxer.start((chunk) => {
        ...
        if (chunk.type === 'key') {
          ...
          assert(
            chunk.timestamp >= lastGroup.timestamp,
            'out of group chunk timestamp'
          );
          lastGroup.length++;
        }
      ...
      console.log(
        `Demuxing done (assuming not async loading), frames = ${this.frames}, Sit.videoFrames = ${Sit.videoFrames}`
      );
  ...
  getGroup(frame) {
    ...
    assert(
      last !== undefined,
      `last groups is undefined, I've loaded ${this.groups.length}`
    );
    console.warn(
      `Last frame = ${last.frame}, length = ${last.length}, i.e. up to ${
        last.frame + last.length - 1
      }`
    );
  ...
  requestFrame(frame) {
    ...
    const addedToDecodeQueue =
      this.decoder.decodeQueueSize - decodeQueueSizeBefore;
  ...
  purgeGroupsExcept(keep) {
    ...
    for (const g in this.groups) {
      const group = this.groups[g];
      if (group.pending > 0) numPending++;
      if (
        keep.find((keeper) => keeper === group) === undefined &&
        group.loaded
      ) {
      ...
      for (const _g in this.groups) {
        ...
        for (let i = g.frame; i < g.frame + g.length; i++) {
          if (
            ...
            this.imageCache[i].width !== 0
          )
            images++;
          if (
            this.imageDataCache[i] !== undefined &&
            this.imageDataCache[i].width !== 0
          )
            imageDatas++;
          if (this.frameCache[i] !== undefined) framesCaches++;
        }

        d += `Group ${_g} f = ${g.frame} l = ${g.length} ts = ${
          g.timestamp
        } i = ${images} id = ${imageDatas} fc = ${framesCaches}${
          g.loaded ? ' Loaded ' : ''
        }${g.pending ? ` pending = ${g.pending}` : ''}<br>`;
      }
    }
  ...
  getImage(frame) {
    ...
    if (frame >= this.lastGetImageFrame) {
      ...
      // going backward
      if (frame > cacheWindow) this.requestFrame(frame - cacheWindow);
    }
    ...
    while (A >= 0 && B < this.chunks.length) {
      if (A >= 0) {
        if (
          this.imageCache[A] !== undefined &&
          this.imageCache[A].width !== 0
        ) {
          bestFrame = A;
          break;
      ...
      if (B < this.chunks.length) {
        if (
          this.imageCache[B] !== undefined &&
          this.imageCache[B].width !== 0
        ) {
          bestFrame = B;
          break;
...
*/
      ...
      const newImageData = new ImageData(newData, this.width, this.height);
  ...
  updateFilter(frameNumber) {
    ...
    if (image) {
      ...
      if (!this.imageDataCache[frameNumber]) {
        ...
        const imageData = this.ctx_tmp.getImageData(
          0,
          0,
          this.width,
          this.height
      ...
      this.filterWorkers[this.nextWorker].postMessage([
        frameNumber,
        clonedImageData,
      ]);
      this.nextWorker++;
      if (this.nextWorker >= this.numWorkers) this.nextWorker = 0;
  ...
  update() {
    ...
    for (const g in this.groups) {
      const group = this.groups[g];
      if (group.pending > 0) par.renderOne = true;
    }
