...
export class CNodeControllerTrackToTrack extends CNodeController {
  ...
  apply(f, objectNode) {
    ...
    const camPos = this.in.sourceTrack.p(f);
...
export class CNodeControllerTrackPosition extends CNodeController {
  ...
  apply(f, objectNode) {
    ...
    assert(
      this.in.sourceTrack !== undefined,
      `CNodeControllerTrackPosition: sourceTrack is undefined, id=${this.id}`
    );
    const pos = this.in.sourceTrack.p(f);
    assert(
      !Number.isNaN(pos.x),
      "CNodeControllerTrackPosition: track's position.x NaN"
    );
...
export class CNodeControllerFocalLength extends CNodeController {
  ...
  apply(f, objectNode) {
    ...
    assert(
      focal_len !== undefined,
      'CNodeControllerFocalLength: focal_len is undefined'
    );
    ...
    assert(
      !Number.isNaN(focal_len),
      'CNodeControllerFocalLength: focal_len is NaN'
    );
    ...
    const camera = objectNode.camera;
    const sensorSize =
      2 * this.referenceFocalLength * tan(radians(this.referenceFOV) / 2);

    const vFOV = degrees(2 * atan(sensorSize / 2 / focal_len));

    camera.fov = vFOV;
    assert(
      !Number.isNaN(camera.fov),
      `CNodeControllerFocalLength: camera.fov is NaN, focal_len=${focal_len} vFOV=${vFOV}`
    );
    camera.updateProjectionMatrix();
...
export class CNodeControllerFOV extends CNodeController {
  ...
  apply(f, objectNode) {
    ...
    assert(
      value !== undefined,
      `CNodeControllerFOV: source.v(f) is undefined, id = ${this.id} f=${f}`
    );
    ...
    if (typeof value === 'number') {
      ...
      assert(
        0,
        "CNodeControllerFOV: no vFOV or misbRow member in source track, can't set FOV"
      );
    }
    camera.updateProjectionMatrix();
  }
...
export class CNodeControllerMatrix extends CNodeController {
  ...
  apply(f, objectNode) {
    ...
    assert(
      ...
      'CNodeControllerMatrix: worldMatrix is not an object'
    );
...
export class CNodeControllerATFLIRCamera extends CNodeController {
  ...
  apply(f, objectNode) {
    ...
    const focalMode = this.in.focalMode.v(f);
    //  var mode = this.in.sensorMode.v(f)
    const zoom = this.in.zoomMode.v(f);
  ...
  recalculate() {
    ...
    for (let f = 0; f < this.frames; f++) {
      if (Math.random() < this.in.frequency.v(f)) {
        const multiply =
          this.in.multiply !== undefined ? this.in.multiply.v(f) : 1;

        velocity.x +=
          (1 / 10000) * this.in.xScale.v(f) * multiply * (Math.random() - 0.5);
        velocity.y +=
          (1 / 10000) * this.in.yScale.v(f) * multiply * (Math.random() - 0.5);
      }
      ...
      const spring = this.in.spring.v(f);
