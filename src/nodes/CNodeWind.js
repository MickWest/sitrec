...
export class CNodeWind extends CNode {
  constructor(v, _guiMenu) {
    ...
    this.guiFrom = this.gui
      .add(this, 'from', 0, 359, 1)
      .name(`${this.name} Wind From`)
      .onChange((x) => this.recalculateCascade());
    this.guiKnots = this.gui
      .add(this, 'knots', 0, this.max, 1)
      .name(`${this.name} Wind Knots`)
      .onChange((x) => this.recalculateCascade());
  ...
  getValueFrame(f) {
    ...
    const wind = getLocalNorthVector(this.position);
    ...
    assert(
      !Number.isNaN(wind.x) && !Number.isNaN(wind.y) && !Number.isNaN(wind.z),
      'Wind vector has NaNs'
    );
...
export class CNodeDisplayWindArrow extends CNode {
  ...
  recalculate() {
    //    var A = Sit.jetOrigin.clone()
    const A = this.in.displayOrigin.p(0);
    const B = A.clone().add(this.in.source.p().multiplyScalar(10000));
    DebugArrowAB(`${this.id} Wind`, A, B, this.arrowColor, true, GlobalScene);
  }
}
