...
export function RLLAToECEFV_Sphere(
  latitude,
  longitude,
  altitude,
  radius = wgs84.RADIUS
...
export function LLAToECEFVD(V) {
  const a = RLLAToECEF(radians(V.x), radians(V.y), V.z);
  return new Vector3(a[0], a[1], a[2]);
}
...
export function LLAToEUS(lat, lon, alt = 0, radius = wgs84.RADIUS) {
  ...
  const enu = ECEF2ENU(ecef, radians(Sit.lat), radians(Sit.lon), radius); // when there's a terrain, Sit.lat/lon is set to the origin in CNodeTerrain
...
export function LLAToEUSMAP(lat, lon, alt, radius = wgs84.RADIUS) {
  ...
  const z = alt - drop(enu.x, enu.y, radius); // use z to avoiud confusion with enu.y

  const eus = V3(enu.x, z, -enu.y); // z is y
  return eus;
}
...
export function LLAToEUSMAPGlobe(lat, lon, alt, radius = wgs84.RADIUS) {
  assert(Sit.lat !== undefined, 'Sit.lat undefined in LLAToEUS');
  const ecef = RLLAToECEFV_Sphere(
    radians(lat),
    radians(lon),
    alt,
    wgs84.RADIUS
  );
  const enu = ECEF2ENU(ecef, radians(Sit.lat), radians(Sit.lon), wgs84.RADIUS); // when there's a terrain, Sit.lat/lon is set to the origin in CNodeTerrain

  // set world y from alt and adjust for drop from map x and y
  //   var z = alt - drop(enu.x,enu.y,radius) // use z to avoiud confusion with enu.y

  const eus = V3(enu.x, enu.z, -enu.y); // z is y
  return eus;
}
...
export function raDecToAzElRADIANS(ra, dec, lat, lon, lst) {
  ...
  const cosAz =
    (Math.sin(dec) - Math.sin(el) * Math.sin(lat)) /
