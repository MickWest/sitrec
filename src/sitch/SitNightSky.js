...
export const SitNightSky = {
  ...
  mainCamera: {
    far: 80000000,
    startCameraPosition: [2032347.51, 19437577.3, 23271391.24],
  ...
  setup2: function () {
    ...
    const labelVideo = new CNodeViewUI({
      id: 'labelVideo',
      overlayView: viewLook,
    });
    AddTimeDisplayToUI(labelVideo, 50, 96, 2.5, '#f0f000');

    const labelMainViewPVS = new CNodeViewUI({
      id: 'labelMainViewPVS',
      overlayView: ViewMan.list.mainView.data,
    });
    labelMainViewPVS.addText(
      'videoLabelp1',
      'L = Lat/Lon from cursor',
      10,
      2,
      1.5,
      '#f0f00080'
    ...
    labelMainViewPVS.addText(
      'videoLabelp3',
      'Drag and drop .txt or .tle files',
      12,
      6,
      1.5,
      '#f0f00080'
    ...
    par.validPct = 100;
    labelMainViewPVS
      .addText('videoLabelInRange', 'xx', 92, 2, 1.5, '#f0f00080')
      .update(function () {
    ...
    if (Globals.userID > 0)
      ...
      this.permaButton = gui
        .add(this, 'loginAttempt')
        .name('Permalink DISABLED (click to log in)');
    }
  },
  ...
  makeNightSkyURL: () => {
    ...
    FileManager.rehostDynamicLinks().then(() => {
      ...
      const p = mainCam.position.clone();
      const v = new Vector3();
      v.setFromMatrixColumn(mainCam.matrixWorld, 2);
      v.multiplyScalar(-1000);
      v.add(p);

      const _q = mainCam.quaternion.clone();
      const q = { x: _q.x, y: _q.y, z: _q.z, w: _q.w };
