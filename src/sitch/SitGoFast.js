import { AlwaysDepth, DirectionalLight, HemisphereLight } from 'three';
import {
  EarthRadiusMiles,
  FileManager,
  guiJetTweaks,
...
export const SitGoFast = {
  ...
  videoView: { left: 0.825, top: 0.6666, width: -1, height: 0.3333 },
  ...
  updateFunction: function (f) {
    ...
    const A = targetNode.p(f);
    const B = jetNode.p(f);
    const rng = NMFromMeters(B.sub(A).length());
    par.rng = rng;

    const Vc = NMFromMeters(
      this.fps * 60 * 60 * closingSpeed(targetNode, jetNode, f)
    );
    par.Vc = Vc;
  },
  ...
  setup: function () {
    ...
    this.GoFastBank = RollingAverage(
      FileManager.get('GoFastBank').map((row) => -Number.parseFloat(row[12])),
      50
    );
    this.GoFastAz = ExpandKeyframes(FileManager.get('GoFastAz'), Sit.frames);
    this.GoFastEl = RollingAverage(
      ExpandKeyframes(FileManager.get('GoFastEl'), Sit.frames),
      50
    );
    this.GoFastRNG = RollingAverage(
      ExpandKeyframes(FileManager.get('GoFastRNG'), Sit.frames),
      50
    );
    ...
    const azEditorNode = new CNodeCurveEditor({
      ...
      editorConfig: {
        ...
        yStep: 5,
        points: [
          0, -35, 373.215, -35.655, 233.258, -40.092, 233.258, -41.773, 524.826,
          -47.886, 522.493, -49.007, 835.059, -54.128, 835.059, -55.249, 1031,
          -58.215, 900, -58.215,
        ],
      },
      frames: Sit.frames,
    ...
    new CNodeMunge({
      id: 'GoFastRNG',
      inputs: {
        NM: new CNodeArray({ id: 'gofastRNGArray', array: this.GoFastRNG }),
      },
      munge: function (f) {
    ...
    const waterTexture = new TextureLoader().load(
      'data/images/28_sea water texture-seamless-dark.jpg?v=1'
    );
