...
const GimbalDefaults = {
  ...
  setup: function () {
    ...
    if (!gui)
      // the rest is unsupported for now in console mode
      return;
    ...
    new CNodeGUIValue(
      {
        id: 'fleetTurnStart',
        value: this.fleetTurnStart,
        start: 0,
        end: 35,
        step: 0.1,
        desc: 'Fleet Turn Start',
      },
      guiTweaks
    );
    new CNodeGUIValue(
      {
        id: 'fleetTurnRate',
        value: this.fleetTurnRate,
        start: 0,
        end: 50,
        step: 0.1,
        desc: 'Fleet Turn Rate',
      },
      guiTweaks
    );
    new CNodeGUIValue(
      {
        id: 'fleetAcceleration',
        value: this.fleetAcceleration,
        start: 1,
        end: 50,
        step: 0.1,
        desc: 'Fleet Acceleration',
      },
      guiTweaks
    );
    new CNodeGUIValue(
    ...
    new CNodeFleeter({
      ...
      turnFrame: new CNodeMunge({
        ...
        frames: 0,
        munge: function (f) {
          return 30 * (this.in.t.v(0) + 2.1);
        },
      }), // when do we start to turn
      offX: -2,
      offY: 0,
      offZ: -1, // offset in NM
    });
    ...
    NodeMan.get('cloudSpeedEditor').editorView.addInput(
      'compare',
    ...
    azEditorNode.editorView.addInput(
      'compare',
      new CNodeGraphSeries({
        id: 'azGraphSources',
        inputs: {
    ...
    new CNodeDisplayTrack({
      ...
      width: 1,
      visibleCheck: () => {
        return ViewMan.get('SAPage').buttonBoxed(16);
      },
    });
...
export const SitGimbalNear = {
  ...
  altitudeLabelFar: {
    ...
    position: 'FARLOSTraverseConstantSpeed',
    defer: true,
  },
...
function SetupAzInputs() {
  ...
  const azEditorNode = new CNodeCurveEditor({
    id: 'azEditor',
    visible: false,
    left: 0.25,
    top: 0.0,
    ...
    editorConfig: {
      ...
      yStep: 10,
      points: [
        0, -54, 592.475, -27.197, 226.259, -43.677, 226.259, -47.601, 496.834,
        -28.925, 496.834, -32.848, 732.43, -13.232, 732.43, -17.156, 891.048,
        -1.614, 891.048, -5.538, 1031, 7, 902.711, -2.243,
      ], // for regression
    },
    frames: Sit.frames,
  });
  ...
  azSourceInputs = {
    ...azSourceInputs,
    'Az Editor': 'azEditor',
    'Az Unsmoothed': new CNodeArray({
      id: 'AzArray',
      array: Sit.CSV.map((row) => -row[4]),
    }),
...
export function SetupGimbal() {
  const azSourceInputs = SetupAzInputs();
  ...
  const elStartNode = makeCNodeGUIValue(
    'elStart',
    -2.01,
    -4,
    0,
  ...
  const elRiseNode = makeCNodeGUIValue(
    'elRise',
    0.025,
    -0.1,
    0.1,
  ...
  new CNodeSwitch(
    {
      id: 'bank',
      inputs: {
        'Recorded Angle': new CNodeArray({
          id: 'recordedAngle',
          array: Sit.CSV.map((row) => -Number.parseFloat(row[3])),
        }),
  ...
  new CNodeSwitch(
    {
      id: 'glareAngle',
      inputs: {
        Unsmoothed: new CNodeNegate({
          id: 'Unsmoothed',
          node: new CNodeArray({
            id: 'unsmoothedArray',
            array: Sit.CSV.map((row) => Number.parseFloat(row[1])),
          }),
        }),
        'Moving Average': new CNodeNegate({
          id: 'movingAverage',
          node: new CNodeArray({
            id: 'movingAverageArray',
            array: Sit.glareAngleSmooth,
          }),
        }),
        'Markus Smoothed': new CNodeNegate({
          id: 'markusSmoothed',
          node: new CNodeArray({
            id: 'markusSmoothedArray',
            array: Sit.CSV.map((row) => Number.parseFloat(row[6])),
          }),
        }),
        MickKeyframe: new CNodeArray({ id: 'MickKeyframe', array: Sit.CSV2 }),
        'Markus Keyframe': new CNodeNegate({
          id: 'markusKeyframe',
          node: new CNodeArray({
            id: 'markusKeyFrameArray',
            array: Sit.CSV.map((row) => Number.parseFloat(row[13])),
          }),
        }),
      },
  ...
  new CNodeNegate({
    id: 'recordedCueAz',
    node: new CNodeArray({
      id: 'recordedCueAzArray',
      array: Sit.CSV.map((row) => Number.parseFloat(row[9])),
    }),
  });
  ...
  const cloudSpeedEditor = new CNodeCurveEditor({
    ...
    editorConfig: {
      ...
      points: [
        0, -0.155, 181.334, -0.132, 433.866, -0.091, 354.555, -0.102, 718.438,
        -0.055, 632.133, -0.069, 1030, -0.013, 897.667, -0.023,
      ],
    },
    frames: Sit.frames,
    visible: true,
  ...
  const turnRateFromCloudsNode = new CNodeTurnRateFromClouds({
  ...
  const jetTrack = new CNodeJetTrack({
    id: 'jetTrack',
    inputs: {
      speed: 'jetTAS',
      altitude: 'jetAltitude',
  ...
  console.log(
    `Setting LocalFrame Position to ${LocalFrame.position.x},${LocalFrame.position.y},${LocalFrame.position.z}`
  );
