...
export class CCustomManager {
  setup() {
    if (Sit.canMod) this.buttonText = 'SAVE MOD';
    else this.buttonText = 'SAVE CUSTOM';
    ...
    if (Globals.userID > 0)
      this.serializeButton = theGUI
        .add(this, 'serialize')
        .name(this.buttonText)
        .setLabelColor(this.buttonColor);
    else
      this.serializeButton = theGUI
        .add(this, 'loginAttempt')
        .name('Export Disabled (click to log in)')
        .setLabelColor('#FF8080');

    this.serializeButton.moveToFirst();
  }

  loginAttempt() {
    FileManager.loginAttempt(
      this.serialize,
  ...
  serialize() {
    ...
    assert(
      Sit.canMod || Sit.isCustom,
      'one of Sit.canMod or Sit.isCustom must be true to serialize a sitch'
    );
    assert(
      !Sit.canMod || !Sit.isCustom,
      'one of Sit.canMod or Sit.isCustom must be false to serialize a sitch'
    );
    ...
    FileManager.rehostDynamicLinks(true).then(() => {
      ...
      NodeMan.iterate((id, node) => {
        if (node.modSerialize !== undefined) {
          ...
          assert(
            nodeMod.rootTestRemove !== undefined,
            `Not incorporating ...super.modSerialzie.  rootTestRemove is not defined for node:${id}Class name ${node.constructor.name}`
          );
          // remove it
          nodeMod.rootTestRemove = undefined;
      ...
      const globalsNeeded = ['showMeasurements'];

      const pars = {};
      for (const key of parNeeded) {
      ...
      const globals = {};
      for (const key of globalsNeeded) {
      ...
      Rehoster.rehostFile(name, str).then((staticURL) => {
        ...
        const customLink = `${SITREC_ROOT}?${paramName}=${staticURL}`;
  ...
  deserialize(sitchData) {
    ...
    if (sitchData.loadedFiles) {
      ...
      for (const id in sitchData.loadedFiles) {
        loadingPromises.push(
          FileManager.loadAsset(Sit.loadedFiles[id], id).then((result) => {
    ...
    Promise.all(loadingPromises).then(() => {
      ...
      if (sitchData.mods) {
        // apply the mods
        for (const id in sitchData.mods) {
          const node = NodeMan.get(id);
          if (node.modDeserialize !== undefined) {
            console.log(
              `Applying mod to node:${id} with data:${sitchData.mods[id]}`
            );
      ...
      // apply the pars
      if (sitchData.pars) {
        for (const key in sitchData.pars) {
      ...
      if (sitchData.globals) {
        for (const key in sitchData.globals) {
          console.warn(
            `Applying global ${key} with value ${sitchData.globals[key]}`
          );
      ...
      // and Sit
      if (sitchData.Sit) {
        for (const key in sitchData.Sit) {
