...
export class CSituation {
  ...
  async loadAssets() {
    ...
    let assets = this.files ?? {};
    const assets2 = this.files2 ?? {};
    if (this.nightSky) {
      assets = { ...assets, ...assets2, ...NightSkyFiles };
    }
    if (!isConsole) infoDiv.innerHTML = 'Loading<br>';
    for (let key in assets) {
      ...
      if (key !== 'videoFile') {
        if (!isConsole) infoDiv.innerHTML += `${assets[key]}<br>`;
        await FileManager.loadAsset(assets[key], key);
      }
    }
    if (!isConsole) infoDiv.innerHTML = 'done loading';
  }
