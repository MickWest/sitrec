...
function inside(p, quad) {
  ...
  for (let q = 0; q < 4; q++) {
    let q1 = q + 1;
    if (q1 === 4) q1 = 0;
    const q0toq1 = quad[q1].clone().sub(quad[q]);
    const q0top = p.clone().sub(quad[q]);
    const dot = q0toq1.y * q0top.x - q0toq1.x * q0top.y;
...
export class CRegionSelector {
  ...
  dumpRect() {
    return `${this.dumpCorner(0)} ${this.dumpCorner(1)} ${this.dumpCorner(
      2
    )} ${this.dumpCorner(3)}`;
  }
  ...
  onMouseDown(view, e, mouseX, mouseY) {
    ...
    if (!this.active) {
      ...
      if (inside(p, this.rect)) this.dragMode = DRAG_MOVE;
      else {
        if (!this.useSkew) this.dragMode = DRAG_ROTATE;
      }
      for (let d = 0; d < this.dragpoints.length; d++) {
  ...
  onMouseDrag(view, e, mouseX, mouseY) {
    ...
    switch (this.dragMode) {
      ...
      case DRAG_ROTATE: {
        ...
        for (let q = 0; q < 4; q++) {
          const corner = this.rectStart[q].clone();
      ...
      case DRAG_POINT:
        if (!this.useSkew) {
          ...
          const drag = this.dragpoints[this.dragIndex];
          const left = this.dragpoints[(this.dragIndex + 1) & 3];
          const opposite = this.dragpoints[(this.dragIndex + 2) & 3];
          const right = this.dragpoints[(this.dragIndex + 3) & 3];

          // get unit vectors from d in left and right directions
          const dl1 = left.point.clone().sub(drag.point).normalize();
          const dr1 = right.point.clone().sub(drag.point).normalize();

          const dd = p.clone().sub(drag.point); // dd is the amount the point has moved.
          const dd1 = dd.clone().normalize();
    ...
    if (
      !clockwiseXY(
        this.dragpoints[0].point,
        this.dragpoints[1].point,
        this.dragpoints[2].point
      )
    ) {
      let t;
    ...
    if (!this.useSkew) {
      ...
      makeBRight(
        this.dragpoints[di].point,
        this.dragpoints[(di + 1) & 3].point,
        this.dragpoints[(di + 2) & 3].point
      );
      makeBRight(
        this.dragpoints[(di + 2) & 3].point,
        this.dragpoints[(di + 3) & 3].point,
        this.dragpoints[di].point
      );
    }
  ...
  render(ctx) {
    ...
    if (this.centerLine) {
      ctx.lineWidth = 0.5;
      ctx.moveTo(
        (this.rect[0].x + this.rect[3].x) / 2,
        (this.rect[0].y + this.rect[3].y) / 2
      );
      ctx.lineTo(
        (this.rect[1].x + this.rect[2].x) / 2,
        (this.rect[1].y + this.rect[2].y) / 2
      );
      ctx.stroke();
    }
    ...
    const arrowSize = 20;
    const mid = this.rect[0].clone().add(this.rect[1]).multiplyScalar(0.5);
    const left = this.rect[0]
      .clone()
      .sub(this.rect[1])
      .normalize()
      .multiplyScalar(arrowSize);
    const up = this.rect[0]
      .clone()
      .sub(this.rect[3])
      .normalize()
      .multiplyScalar(arrowSize);
    const pLeft = mid.clone().add(left);
    const pRight = mid.clone().sub(left);
    const pPeak = mid.clone().add(up);
