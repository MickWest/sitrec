...
export class CUnits {
  constructor(_units, gui) {
    ...
    if (guiPhysics)
      guiPhysics
        .add(this, 'unitsName', this.selectableUnits)
        .name('Units')
        .listen()
  ...
  changeUnits(_units, updateGUI = true) {
    ...
    if (updateGUI) {
      ...
      for (const [unitName, unit] of Object.entries(this.selectableUnits)) {
  ...
  withUnits(m, decimals = 0, unitSize = 'big') {
    if (unitSize === 'big') return this.bigWithUnits(m, decimals);
