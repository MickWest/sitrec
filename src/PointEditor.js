...
import {
  mouseInViewOnly,
  mouseToViewNormalized,
  ViewMan,
} from './nodes/CNodeView';
...
export class PointEditor {
  ...
  adjustUp(height, cameraTrack) {
    for (let i = 0; i < this.positions.length; i++) {
      const frame = this.frameNumbers[i];
      const cameraPos = cameraTrack.p(frame);
      const toCamera = cameraPos.clone().sub(this.positions[i]).normalize();
      const scale = height / toCamera.y;
  ...
  onPointerMove(event) {
    ...
    if (mouseInViewOnly(view, event.clientX, event.clientY)) {
      const [px, py] = mouseToViewNormalized(
        view,
        event.clientX,
        event.clientY
      );
  ...
  getPointFrame(f) {
    ...
    assert(
      this.frameNumbers.length > 0,
      "Can't work with zero frame in a spline"
    );
    ...
    const numFramesCovered =
      this.frameNumbers[lastIndex] - this.frameNumbers[0];
    let segment = 0;
    let t = 0;
    ...
    if (segment === lastIndex) {
      ...
      t =
        t +
        (tPerSegment * (f - this.frameNumbers[segment])) /
          (this.frameNumbers[segment + 1] - this.frameNumbers[segment]);
    }
    //        if (f===3015)
    //            console.log("f:"+f+", t:"+t)
  ...
  insertPoint(frame, position) {
    ...
    if (this.frameNumbers.length === 0 || this.frameNumbers[0] > frame) {
      ...
      let insertPoint = 0;
      while (
        !(insertPoint === this.frameNumbers.length - 1) &&
        !(
          this.frameNumbers[insertPoint] <= frame &&
          this.frameNumbers[insertPoint + 1] > frame
        )
      ) {
        insertPoint++;
      }
