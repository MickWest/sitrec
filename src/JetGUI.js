...
export function updateGUIFrames() {
  ...
  gui
    .add(par, 'time', 0, Sit.frames / Sit.fps, 1 / Sit.fps)
    .onChange(UIChangedTime)
    .listen()
    .name('Time (sec)');
  gui
    .add(par, 'frame', 0, Sit.frames - 1, 1)
    .onChange(UIChangedFrame)
    .listen()
    .name('Frame in Video');
...
export function SetupJetGUI() {
  guiTweaks
    .add(Sit, 'aFrame', 0, Sit.frames - 1, 1)
    .listen()
    .onChange(() => {
  ...
  guiTweaks
    .add(Sit, 'bFrame', 0, Sit.frames - 1, 1)
    .listen()
    .onChange(() => {
  ...
  guiJetTweaks
    .add(par, 'podPitchPhysical', -20, 150, 1)
    .onChange(UIChangedPR)
    .listen()
    .name('Pod (Ball) Pitch');
  guiJetTweaks
    .add(par, 'podRollPhysical', -180, 180, 1)
    .onChange(UIChangedPR)
    .listen()
    .name('Pod Head Roll');
  guiJetTweaks
    .add(par, 'deroFromGlare')
    .listen()
    .name('Derotation = Glare Angle')
  ...
  guiTweaks
    .add(par, 'glareStartAngle', 40, 80, 0.1)
    .listen()
    .name('Glare Start Angle')
    .onChange(curveChanged);
  guiTweaks
    .add(par, 'initialGlareRotation', 0, 20, 0.1)
    .listen()
    .name('Glare Initial Rotation')
    .onChange(() => {
      calculateGlareStartAngle();
      curveChanged();
    });
  ...
  toggler(
    'b',
    guiTweaks
      .add(par, 'podWireframe')
      .listen()
      .name('[B]ack Pod Wireframe')
      .onChange((value) => {
  ...
  toggler(
    'k',
    guiShowHide
      .add(par, 'showKeyboardShortcuts')
      .listen()
      .name('[K]eyboard Shortcuts')
      .onChange((value) => {
        if (value) {
  ...
  toggler(
    'x',
    guiShowHide
      .add(par, 'showGlareGraph')
      .listen()
      .name('Sh[o]w Glare Graph')
      .onChange(curveChanged)
  );
  ...
  togglerNodes(
    ...
    [
      'azEditorView',
      'speedGraphView',
      'cloudSpeedEditorView',
      'tailAngleGraphView',
      'speedGraphFleetView',
      'altitudeGraphView',
      'sizePercentGraphView',
    ],
    guiShowHide,
    '[D]eclutter]',
    curveChanged
  ...
  guiTweaks
    .add(par, 'jetOffset', f2m(-30), f2m(10), 0.01)
    .listen()
    .name('Jet Y offset')
    .onChange((value) => (FA18.position.y = value));
  guiTweaks
    .add(par, 'TAS', 340, 360, 0.01)
    .listen()
    .name('TAS True Airspeed')
    .onChange(curveChanged);
  guiTweaks
    .add(par, 'integrate', 1, 100, 1)
    .listen()
