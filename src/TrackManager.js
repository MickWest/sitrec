...
import {
  CNodeControllerMatrix,
  CNodeControllerTrackPosition,
} from './nodes/CNodeControllerVarious';
import { MISB } from './MISBUtils';
import { isNumber } from 'mathjs';
import {
  CNodeMISBDataTrack,
  makeLOSNodeFromTrack,
  removeLOSNodeColumnNodes,
} from './nodes/CNodeMISBData';
...
export function addTracks(
  ...
  for (const trackFileName of trackFiles) {
    ...
    const trackOb = TrackManager.add(
      trackID,
      new CSitrecTrack(trackFileName, trackDataNode, trackNode)
    );
      ...
      for (const dropTargetSwitch of dropTargets) {
        if (NodeMan.exists(dropTargetSwitch)) {
          ...
          if (Sit.dropAsController) {
            ...
            switchNode.addOption(
              menuText,
              new CNodeControllerTrackPosition({
                sourceTrack: trackID,
              })
            );
            // and select it
    ...
    if (isNumber(value)) {
      ...
      value.misbRow !== undefined &&
      isNumber(Number(value.misbRow[MISB.SensorVerticalFieldofView]))
    ) {
      hasFOV = true;
    ...
    if (
      hasFOV &&
      Sit.dropTargets !== undefined &&
      Sit.dropTargets.fov !== undefined
    ) {
    ...
    if (
      value.misbRow !== undefined &&
      isNumber(value.misbRow[MISB.SensorVerticalFieldofView])
    ) {
      hasAngles = true;
    }
    ...
    if (
      hasAngles &&
      Sit.dropTargets !== undefined &&
      Sit.dropTargets.angles !== undefined
    ) {
      const data = {
        smooth: 120, // maybe GUI this?
      };
      const anglesNode = makeLOSNodeFromTrack(trackID, data);
      trackOb.anglesNode = anglesNode;
      const anglesID = `Angles_${trackFileName}`;
      const anglesController = new CNodeControllerMatrix({
        id: anglesID,
        source: anglesNode,
      });
    ...
    if (centerID !== null) {
      trackOb.displayCenterDataNode = new CNodeDisplayTrack({
        ...
        color: new CNodeConstant({
          id: `colorCenterData_${trackFileName}`,
