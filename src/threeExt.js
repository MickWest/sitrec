...
export function dispose(a) {
  if (a !== undefined) a.dispose();
}
...
function sphereAt(x, y, z, radius, color, parent) {
...
function boxAt(x, y, z, xs, ys, zs, color, parent) {
...
export function DebugWireframeSphere(
  name,
  origin,
  radius,
  color,
  ...
  if (parent === undefined) parent = GlobalScene;
...
export function DebugArrow(
  ...
  if (DebugArrows[name] === undefined) {
    color = new Color(color); // convert from whatever format, like "green" or "#00ff00" to a THREE.Color(r,g,b)
    DebugArrows[name] = new ArrowHelper(
      dir,
      origin,
      _length,
      color,
    ...
    assert(
      parent === DebugArrows[name].parent,
      `Parent changed on debug arrow: was ${DebugArrows[name].parent.debugTimeStamp} now ${parent.debugTimeStamp}`
    );
...
export function scaleArrows(camera) {
  ...
  for (const key in DebugArrows) {
    ...
    const headPosition = arrow.position
      .clone()
      .add(arrow.direction.clone().multiplyScalar(arrow.length));
    const distance = campos.distanceTo(headPosition);
...
export function DebugAxes(name, position, length) {
  DebugArrow(
    `${name}Xaxis`,
    V3(1, 0, 0),
    position.clone().sub(V3(length / 2, 0, 0)),
    length,
    '#FF8080'
  );
  DebugArrow(
    `${name}Yaxis`,
    V3(0, 1, 0),
    position.clone().sub(V3(0, length / 2, 0)),
    length,
    '#80FF80'
  );
  DebugArrow(
    `${name}Zaxis`,
    V3(0, 0, 1),
    position.clone().sub(V3(0, 0, length / 2)),
    length,
    '#8080FF'
  );
}
...
export function DebugMatrixAxes(name, position, matrix, length) {
  ...
  DebugArrow(
    `${name}Xaxis`,
    x,
    position.clone().sub(x.clone().multiplyScalar(length / 2)),
    length,
    '#FF8080'
  );
  DebugArrow(
    `${name}Yaxis`,
    y,
    position.clone().sub(y.clone().multiplyScalar(length / 2)),
    length,
    '#80FF80'
  );
  DebugArrow(
    `${name}Zaxis`,
    z,
    position.clone().sub(z.clone().multiplyScalar(length / 2)),
    length,
    '#8080FF'
  );
}

function DebugArrowOrigin(
  name,
  direction,
  length,
  color,
...
export function intersectMSL(point, heading) {
  ...
  if (intersectSphere2(ray, globe, sphereCollision)) return sphereCollision;
  return null;
}
...
export function testTextureCube(url, position, size, scene) {
  console.log(
    `Creating texture cube at ${position.x},${position.y},${position.z} with size ${size} and texture ${url}`
  );
