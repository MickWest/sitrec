...
export function SituationSetupFromData(sitData, runDeferred) {
  ...
  for (let key in sitDataExpanded) {
    ...
    assert(
      typeof dataDeferred === 'boolean',
      'SituationSetup: _data.defer must be a boolean'
    );
    // assert runDeferred is a boolean
    assert(
      typeof runDeferred === 'boolean',
      'SituationSetup: runDeferred must be a boolean'
    );
    ...
    const node = SetupFromKeyAndData(key, _data);
  ...
  for (const subKey in data) {
    if (typeof data[subKey] === 'object') {
      if (data[subKey].kind !== undefined) {
        ...
        const anonResult = SetupFromKeyAndData(
          undefined,
          data[subKey],
          depth + 1
        );
        ...
        assert(
          anonResult instanceof CNode,
          `SituationSetup: anonymous object must be a CNode derived object. Kind = ${
            data[subKey].kind
          }\n${JSON.stringify(data)}`
        );
...
export function SetupFromKeyAndData(key, _data, depth = 0) {
  ...
  if (data.kind !== undefined) {
    ...
    if (depth === 0) {
      assert(
        data.id === undefined,
        'SituationSetup: data.id is deprecated, use key as id'
      );
  ...
  switch (key) {
    ...
    case 'mainCamera': {
      ...
      if (
        data.startCameraPosition === undefined &&
        data.startCameraPositionLLA === undefined
      ) {
        data = {
          ...data,
          ...{
      ...
      gui
        .add(cameraNode.camera, 'fov', 0.35, 80, 0.01)
        .onChange((value) => {
    ...
    case 'lookCamera': {
      ...
      if (cameraID === 'lookCamera') {
        ...
        if (data.addFOVController) {
          gui
            .add(lookCameraNode.camera, 'fov', 0.35, 80, 0.01)
            .onChange((value) => {
              lookCameraNode.camera.updateProjectionMatrix();
            })
            .listen()
            .name('Look Camera FOV');
        }
      }
    ...
    case 'cameraTrack': {
      SSLog();
      assert(
        Sit.lat !== undefined && Sit.lon !== undefined,
        'SituationSetup: cameraTrack needs Sit.lat and Sit.lon defined. i.e. after terrain'
      );
    ...
    case 'lookView': {
      ...
      node = lookView;
      if (!data.noOrbitControls) lookView.addOrbitControls(Sit.renderer);
      break;
    }
    ...
    case 'targetSpline': {
      ...
      assert(
        mainView !== undefined,
        'SituationSetup: targetSpline needs a mainView defined'
      );
    ...
    case 'startDistanceFeet':
      SSLog();
      node = new CNodeScale(
        data.id ?? 'startDistance',
        scaleF2M,
        new CNodeGUIValue({ id: 'startDistanceFeet', ...data }, data.gui ?? gui)
      );
      break;
    ...
    case 'ptz': {
      ...
      console.log(
        `MAKE PTZ lookCamera, quaternion = ${
          NodeMan.get('lookCamera').camera.quaternion.x
        }`
      );
    ...
    case 'arrayDataPTZ': {
      ...
      if (data.labelView !== undefined) {
        ...
        labelView
          .addText('alt', '---', 0, 5, 5, '#FFFFFF', 'left')
          .listen(par, 'cameraAlt', function (value) {
        ...
        labelView.addLine('---').update(function (value) {
          this.text = `Pitch ${NodeMan.get('pitchCol')
            .v(par.frame)
            .toFixed(2)}°`;
        });
      }
    ...
    case 'smoothTrack':
      ...
      if (data.method === 'moving') {
        node = NodeFactory.reinterpret(
          data.track,
          'SmoothedPositionTrack',
          {
            //   source: data.track,
            method: data.method,
            copyData: true,
    ...
    case 'azSlider':
      if (data) {
        ...
        gui
          .add(par, 'az', aZMin, aZMax, 0.2)
          .listen()
          .onChange(UIChangedAz)
          .name('azimuth');
      }
      break;
    ...
    case 'arrayFromKeyframes': {
      ...
      if (data.smooth) {
        node = NodeFactory.reinterpret(
          data.id,
          'SmoothedArray',
          { source: data.id, window: data.smooth },
          'source'
        );
      }
      break;
    ...
    case 'parWatch':
      SSLog();
      assert(
        par[data.watchID] !== undefined,
        'SituationSetup: parWatch needs a valid watchID that references a variable in par'
      );
    ...
    case 'addGraphSeries': {
      ...
      assert(
        ...
        'SituationSetup: addGraphSeries needs a source object'
      );
      assert(
        data.graph !== undefined,
        'SituationSetup: addGraphSeries needs a graph object'
      );
      const graph = NodeMan.get(data.graph);
      assert(
        graph instanceof CNodeCurveEditor,
        'SituationSetup: addGraphSeries needs a valid graph object'
      );
      graph.editorView.addInput(
        `${data.id}_view`,
    ...
    case 'segmentSelect':
      ...
      gui.add(par, 'segment', data.menu).onChange((v) => {
    ...
    default:
      ...
      if (NodeFactory.isController(`Controller${key}`)) {
        ...
        if (objectNode) {
          ...
          console.error(
            `SituationSetup: controller ${key} needs an object/camera`
          );
        }
      } else {
