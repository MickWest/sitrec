...
if (urlParams.get('custom')) {
  ...
  await fetch(customSitch, { mode: 'cors' })
  ...
  // customSitch is the URL of a sitch definition file
  // fetch it, and then use that as the sitch
  await fetch(modSitch, { mode: 'cors' })
    .then((response) => response.text())
    .then((data) => {
      ...
      let sitchObject = SitchMan.findFirstData((s) => {
        return s.data.name === modObject.modding;
      });

      assert(
        sitchObject !== undefined,
        `Modding sitch not found: ${modObject.modding}`
      );
...
async function newSitch(situation, customSetup = false) {
  const url = `${SITREC_ROOT}?sitch=${situation}`;
  ...
  console.log(
    `Setting up new sitch: ${situation} Sit.menuName = ${Sit.menuName} Sit.name = ${Sit.name}`
  );
...
async function initializeOnce() {
  ...
  let textSitches = [];
  await fetch(`${SITREC_SERVER}getsitches.php`, { mode: 'cors' })
    .then((response) => response.text())
    .then((data) => {
  ...
  SitchMan.iterate((key, sitch) => {
    if (sitch.hidden !== true && sitch.menuName !== undefined) {
      if (isLocal && sitch.isSitKML) sitch.menuName = `${sitch.menuName} (KML)`;
  ...
  const _gui = Globals.menuBar
    .addFolder(process.env.BUILD_VERSION_STRING)
    .close()
    .perm();
  const _guiPhysics = Globals.menuBar.addFolder('Physics').close().perm();
  const _guiShowHide = Globals.menuBar.addFolder('Show/Hide').close().perm();
  const _guiShowHideViews = _guiShowHide.addFolder('Views').close().perm();

  const _guiTweaks = Globals.menuBar
    .addFolder('Tweaks & Effects')
    .close()
    .perm();
  ...
  _gui
    .add(par, 'name', selectableSitches)
    .name('Sitch')
    .perm()
    .listen()
    .onChange((sitch) => {
      console.log(`SITCH par.name CHANGE TO: ${sitch} ->${par.name}`);
      const url = `${SITREC_ROOT}?sitch=${sitch}`;

      const nextSitch = sitch;
      ...
      if (sitch === 'testhere') {
        ...
        for (const key in selectableSitches) {
          if (skip) {
            if (selectableSitches[key] === situation) skip = false;
          } else {
            if (
              selectableSitches[key] !== 'testall' &&
              selectableSitches[key] !== 'testhere'
            )
...
function initRendering() {
  ...
  try {
    ...
    console.error(
      `Incompatible Browser or Graphics Acceleration Disabled\n Error creating WebGLRenderer: ${e}`
    );
    // show an alert
    alert(
...
function legacySetup() {
  ...
  assert(
    Sit.name.slice().toLowerCase() === Sit.name,
    `Sit.name (${Sit.name}) is not all lower case`
  );
...
async function setupFunctions() {
  ...
  if (urlParams.get('data')) {
    ...
    if (urlObject.rhs && Sit.files.starLink !== undefined) {
      Sit.files.starLink = undefined;
      FileManager.rehostedStarlink = true;
      console.log(
        `Deleted starLink from sit, as urlObject.rhs = ${urlObject.rhs}`
      );
    }
  ...
  if (Sit.useGlobe) {
    ...
    par.globe = addAlignedGlobe(
      Sit.globeScale ?? (Sit.terrain !== undefined ? 0.9999 : 1.0)
    );
...
function renderMain() {
  ...
  if (Sit.animated) {
    ...
    updateFrame();
    if (lastFrame !== par.frame) par.renderOne = true;
  }
  ...
  if (Sit.jetStuff && Sit.showGlare) {
    if (glareSprite) {
      glareSprite.position.set(
        targetSphere.position.x,
        targetSphere.position.y,
        targetSphere.position.z
      );

      if (!glareSprite.visible) targetSphere.layers.enable(LAYER.podsEye);
      else targetSphere.layers.disable(LAYER.podsEye);
    }
  }
...
function selectInitialSitch(force) {
  ...
  const startSitch = SitchMan.findFirstData((s) => {
    return lower === s.data.name;
  });
  ...
  Globals.geolocationPromise = new Promise((resolve, reject) => {
    ...
    watchID = navigator.geolocation.watchPosition(
      (position) => {
        ...
        if (!Sit.localLatLon) {
          console.warn(
            'Local lat/lon not enabled in this situation. Aborting geolocation.'
          );
        ...
        Sit.lat = Number.parseFloat(lat.toFixed(2));
        Sit.lon = Number.parseFloat(long.toFixed(2));
        Sit.fromLat = Sit.lat;
        Sit.fromLon = Sit.lon;

        console.log(
          'RESOLVED Local Lat, Lon =',
          Sit.lat,
          Sit.lon,
          ` situation = ${Sit.name}`
        );
...
function disposeEverything() {
  ...
  console.log(
    `GlobalScene originally has ${GlobalScene.children.length} children`
  );
  disposeScene(GlobalScene);
  console.log(
    `GlobalScene now (after dispose) has ${GlobalScene.children.length} children`
  );
  ...
  assert(
    ViewMan.size() === 0,
    `ViewMan.size() should be zero, it's ${ViewMan.size()}`
  );
...
async function waitForParsingToComplete() {
  console.log(
    `Waiting for parsing to complete... Globals.parsing = ${Globals.parsing}`
  );
...
async function waitForTerrainToLoad() {
  console.log(
    `Waiting for terrain loading to complete... Globals.loadingTerrain = ${Globals.loadingTerrain}`
  );
  // Use a Promise to wait
  await new Promise((resolve, reject) => {
    // Function to check the value of Globals.parsing
    function checkloadingTerrain() {
      if (!Globals.loadingTerrain) {
        ...
        console.log(
          `Still Checking, Globals.loadingTerrain = ${Globals.loadingTerrain}`
        );
      }
    }
