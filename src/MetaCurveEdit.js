...
function BezierXfromY(ps, y) {
  ...
  if (y < ps[0].y) return ps[0].x;
  if (y > ps[l].y) return ps[l].x;
...
class MetaBezierCurveEditor {
  constructor(p) {
    ...
    this.curve.ps.push(
      new Point(
        this.C2DX(this.c.clientWidth / 2 - 15),
        this.C2DY(this.c.clientHeight / 2 - 80)
      )
    ); //
    this.curve.ps.push(
      new Point(
        this.C2DX(this.c.clientWidth / 2 - 10),
        this.C2DY(this.c.clientHeight / 2 - 60)
      )
    );
    this.curve.ps.push(
      new Point(
        this.C2DX(this.c.clientWidth / 2 + 5),
        this.C2DY(this.c.clientHeight / 2 + 30)
      )
    );
    this.curve.ps.push(
      new Point(
        this.C2DX(this.c.clientWidth / 2 - 5),
        this.C2DY(this.c.clientHeight / 2)
      )
    );
  ...
  C2DX(canvas_x) {
    return (
      this.min.x +
      ((canvas_x - this.g.x) * (this.max.x - this.min.x)) / this.g.w
    );
  }
  ...
  C2DY(canvas_y) {
    return (
      this.min.y +
      ((this.g.h - (canvas_y - this.g.y)) * (this.max.y - this.min.y)) /
        this.g.h
    );
  }
  ...
  D2CX(data_x) {
    return (
      this.g.x + ((data_x - this.min.x) * this.g.w) / (this.max.x - this.min.x)
    );
  }
  ...
  D2CY(data_y) {
    return (
      this.g.y +
      this.g.h -
      ((data_y - this.min.y) * this.g.h) / (this.max.y - this.min.y)
    );
  }
  ...
  getX(y) {
    ...
    const l = this.curve.ps.length;
    if (y > this.curve.ps[l - 2].y) {
      const x =
        this.curve.ps[l - 2].x +
        (y - this.curve.ps[l - 2].y) * this.topGradient;
      return x;
    }
  ...
  insideGraph(x, y) {
    ...
    return (
      x >= this.D2CX(this.min.x) &&
      y <= this.D2CY(this.min.y) &&
      x <= this.D2CX(this.max.x) &&
      y >= this.D2CY(this.max.y)
    );
  }
  ...
  recalculate() {
    if (this.useRegression) {
      const data = [];
      let n = 0;
      for (let i = 0; i < this.curve.ps.length; i += 2) {
    ...
    lines.forEach((line) => {
      ...
      if (line.y !== undefined) {
        ...
        if (line.x2 !== undefined) {
          ctx.rect(
            this.D2CX(line.x),
            this.D2CY(this.min.y),
            this.D2CX(line.x2) - this.D2CX(line.x),
            this.D2CY(this.max.y) - this.D2CY(this.min.y)
          );
          ctx.fill();
          ctx.stroke();
        } else {
  ...
  update() {
    ...
    if (this.clampFirstY) this.curve.ps[0].y = 0;

    if (this.topGradient !== 0) {
      this.curve.ps[len - 1].x =
        this.curve.ps[len - 2].x +
        this.topGradient *
          (this.curve.ps[len - 1].y - this.curve.ps[len - 2].y);
    }
...
*/
    ...
    assert(
      this.yStep > 0,
      `MetaBezierCurveEditor needs positive non-zero yStep, but it's ${this.yStep}`
    );
    ...
    for (let y = this.min.y; y < this.max.y + 1; y += this.yStep) {
      ...
      const y2 = Number.parseFloat(y.toFixed(2));
    ...
    for (let i = 0; i < pointsToDraw; i++) {
      if (i % 2 === 0) {
        if (!this.override && !this.useRegression) {
          ...
          ctx.moveTo(
            this.D2CX(this.curve.ps[i].x),
            this.D2CY(this.curve.ps[i].y)
          );
          ctx.lineTo(
            this.D2CX(this.curve.ps[i + 1].x),
            this.D2CY(this.curve.ps[i + 1].y)
          );
          ctx.stroke();
        }
        color = 'black';
    ...
    if (this.curve.ps.length > 0) {
      ...
      if (!this.override && !this.useRegression) {
        ...
        ctx.lineTo(
          this.D2CX(this.curve.ps[this.curve.ps.length - 2].x),
          this.D2CY(this.curve.ps[this.curve.ps.length - 2].y)
        );
        ctx.stroke();
      }
    }
    ...
    if (this.compareNode) {
      let nodes;
      if (Array.isArray(this.compareNode)) nodes = this.compareNode;
      else nodes = [this.compareNode];
      ...
      if (this.dynamicY) {
        ...
        if (range !== undefined) {
          const spread = realMaxY - realMinY;
          if (spread < range) {
            const mid = (realMaxY + realMinY) / 2;
        ...
        assert(
          ...
          `min and max y the same! = ${this.min.y}`
        );
    ...
    if (this.p.xLabelDelta) {
      ...
      const deltaInfo = `${delta > 0 ? '+' : ''}${delta.toFixed(0)}`;
...
*/
  ...
  selectPointAt(x, y) {
    ...
    for (let i = 0; i < len; i++) {
      const d = Math.sqrt(
        (this.D2CX(this.curve.ps[i].x) - x) ** 2 +
          (this.D2CY(this.curve.ps[i].y) - y) ** 2
      );
      if (d <= 10) {
  ...
  mouseDown(e) {
    ...
    if (!this.override && !this.disable) {
      ...
      this.curve.ps.push(
        new Point(this.C2DX(e.layerX), this.C2DY(e.layerY + 25))
      );
    ...
    if (!this.override && !this.disable) {
      ...
      if (this.selectedPoint /* && this.insideGraph(e.layerX, e.layerY)*/) {
        ...
        if (
          this.clampXEnds &&
          (this.selectedPointIndex === 0 ||
            this.selectedPointIndex === this.curve.ps.length - 2)
        ) {
          // -2 as the last one is the control point
          move_x = 0;
        }
        ...
        if (this.keepYOrder && this.selectedPointIndex % 2 === 0) {
          if (
            ...
            this.selectedPoint.y + move_y >
              this.curve.ps[this.selectedPointIndex + 2].y
          ) {
            move_y = 0;
          } else {
            if (
        ...
        if (
          this.topGradient !== 0 &&
          this.selectedPointIndex === this.curve.ps.length - 1
        ) {
      ...
      for (let i = 0; i < len; i++) {
        if (i % 2 === 0 || !this.useRegression) {
          const d = Math.sqrt(
            (this.D2CX(this.curve.ps[i].x) - e.layerX) ** 2 +
              (this.D2CY(this.curve.ps[i].y) - e.layerY) ** 2
          );
          if (d <= 10) {
            ctx.fillStyle = 'green';
            if (i % 2 === 0) {
