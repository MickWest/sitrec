...
export function getHumanHorizonFromPitchRollAzEl(jetPitch, jetRoll, az, el) {
  ...
  const relative_AzElHeading = EA2XYZ(el, az, 1)
...
export function getGlareAngleFromFrame(f) {
  if (!NodeMan.exists('glareAngle')) {
    if (f === 0)
      console.warn(
        'GlareAngleFromFrame being called BUT missing glareAngle node'
      );
    return 0;
  }
  ...
  if (f < 698) {
    const old = Number.parseFloat(NodeMan.get('glareAngle').getValueFrame(0)); // old flat line
...
export function update_ERROR_circle(scence, circleCenter) {
  ...
  if (ERROR_circle !== undefined) oldErrorCircleVisible = ERROR_circle.visible;
...
export const vizRadius = 100;
const debugText = ''; // stick text in here, and it's show instead of keyboard shortcuts
 ...
 */
  ...
  if (
    ViewMan.list.video.data.videoPercentLoaded > 0 &&
    ViewMan.list.video.data.videoPercentLoaded < 100
  ) {
  ...
  if (debugText !== '') infoDiv.innerHTML = debugText;
  else infoDiv.innerHTML = keyInfo;
...
export function ChangedPR() {
  ...
  // INPUT
  const localUp = getLocalUpVector(
    LocalFrame.position,
    metersFromMiles(NodeMan.get('radiusMiles').v0)
  );
  ...
  const glarePos = PRJ2XYZ(
    par.podPitchPhysical,
    par.podRollPhysical + NodeMan.get('bank').v(par.frame),
    jetPitchFromFrame(),
    vizRadius
  );
  ...
  const targetPos = PRJ2XYZ(
    par.podPitchIdeal,
    par.podRollIdeal + NodeMan.get('bank').v(par.frame),
    jetPitchFromFrame(),
    vizRadius
  );
...
export function UpdatePRFromEA() {
  ...
  } else par.podRollPhysical = par.podRollIdeal;
  ChangedPR();
}
...
export function SetupTrackLOSNodes() {
  ...
  new CNodeDisplayTrack({
    ...
    color: new CNodeConstant({
      id: 'jetTrackColor',
      value: new Color(0, 1, 1),
    }),
    secondColor: new CNodeConstant({
      id: 'jetTrackColor2',
      value: new Color(0, 0.75, 0.75),
    }),
    width: 3,
  ...
  new CNodeDisplayTrack({
    ...
    inputs: {
      ...
      secondColor: new CNodeConstant({
        id: 'losTraverseColor2',
        value: new Color(0, 0.75, 0),
      }),
...
export function SetupTraverseNodes(
  id,
  traverseInputs,
  defaultTraverse,
  los = 'JetLOS',
...
export function CreateTraverseNodes(idExtra = '', los = 'JetLOS') {
  ...
  new CNodeLOSTraverse({
    ...
    VcMPH: new CNodeGUIValue(
      {
        id: `targetVCGUI${idExtra}`,
        value: 20,
        start: -500,
        end: 500,
        step: 0.01,
    ...
    new CNodeMunge(
      {
        ...
        inputs: {
          ...
          relativeHeading: 'targetRelativeHeading',
        },
        munge: function (f) {
          let newHeading =
            this.in.initialHeading.getHeading() + this.in.relativeHeading.v0;
...
export function MakeTraverseNodesMenu(
  id,
  traverseInputs,
  defaultTraverse,
  idExtra = '',
  ...
  const traverseInputs2 = {};
  for (const inputID in traverseInputs) {
    traverseInputs2[inputID] = traverseInputs[inputID] + idExtra;
  }

  const nodeMenu = new CNodeSwitch(
    {
      id: id,
...
export function initViews() {
  ...
  if (1 || !isLocal) {
    const labelMainView = new CNodeViewUI({
      id: 'labelMainView',
      overlayView: ViewMan.list.mainView.data,
    });
    labelMainView.addText(
      'videolabel1',
      'WORK IN PROGRESS',
      45,
      90,
  ...
  if (
    ...
    // a grid spaced one Nautical mile square
    const gridSquaresGround = 200;
    const gridHelperGround = new GridHelperWorld(
      1,
      metersFromNM(gridSquaresGround),
      gridSquaresGround,
      metersFromMiles(EarthRadiusMiles),
      0x606000,
      0x606000
    );
    GlobalScene.add(gridHelperGround);
  ...
  const line_materialRED = new LineBasicMaterial({
    color: 0xff8080,
    linewidth: 5,
  });
...
export function CommonJetStuff() {
  ...
  if (Sit.name === 'gimbal') {
    AddTailAngleGraph(null, { left: 0.73, top: 0.25, width: -1, height: 0.25 });
  }
...
export function initJetStuff() {
  ...
  view.preRenderFunction = () => {
    ...
    const windVelocityScaledLocal = windTrackLocal
      .v(par.frame)
      .multiplyScalar(vScale);
    const windVelocityScaledTarget = windTrackTarget
      .v(par.frame)
      .multiplyScalar(vScale);

    let jetPosition = ufoTrack.p(par.frame);
    let jetVelocityScaled = trackVelocity(ufoTrack, par.frame).multiplyScalar(
      vScale
    );
    let groundVelocityEnd = jetPosition.clone().add(jetVelocityScaled);
    let airVelocityEnd = groundVelocityEnd
      .clone()
      .sub(windVelocityScaledTarget);
    DebugArrowAB(
      'UFO Ground V',
      jetPosition,
      groundVelocityEnd,
    ...
    jetVelocityScaled = trackVelocity(jetTrack, par.frame).multiplyScalar(
      vScale
    );
  ...
  const podCamera = new PerspectiveCamera(
    45,
  ...
  const viewPod = new CNodeView3D({
    id: 'podBackView',
    visible: false,
    top: 0.010319917440660475,
    left: 0.6583333333333333,
    width: 0.2,
    height: 0.3993808049535604,
  ...
  const podsEyeCamera = new PerspectiveCamera(
    20,
    window.innerWidth / window.innerHeight,
    99,
    farClipLook
  ...
  const podsEyeCameraNode = new CNodeCamera({
    id: 'podsEyeCamera',
  ...
  const ui = new CNodeViewUI({
    id: 'podseye',
    overlayView: ViewMan.list.podsEyeView.data,
  });
  ui.addText('info', 'Pods-Eye View', 50, 90, 6, '#FFFF00');

  // Pod's eye, same, but derotated so horizon is correct.
  const podsEyeDeroCamera = new PerspectiveCamera(
    20,
    window.innerWidth / window.innerHeight,
    99,
    farClipLook
  ...
  const podsEyeDeroCameraNode = new CNodeCamera({
    id: 'podsEyeDeroCamera',
  ...
  new CNodeView3D({
    ...
    preRenderFunction: function () {
      ...
      if (Sit.showGlare) {
        glareSprite.scale.setScalar(0.04);
        glareSprite.material.rotation = radians(
          -par.podRollPhysical + par.glareStartAngle
        );
      }
  ...
  VG('lookView').preRenderFunction = function () {
    ...
    if (Sit.showGlare) {
      ...
      glareSprite.material.rotation = radians(
        -deroNeeded + par.glareStartAngle
      );
    }
  };

  VG('lookView').postRenderFunction = () => {
    if (Sit.showGlare) {
      glareSprite.material.rotation = 0;
    }
  };
